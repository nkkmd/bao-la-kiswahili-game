...
const DEFAULT_OUTPUT = path.join(ROOT, "artifacts/local/reeoe-stage1-development/production-result.json");
...
function strictRawState(state, label = "state") {
  ...
  if (state.phase !== "mtaji") throw new Error(`Development root phase must be mtaji:${label}`);
  if (state.winner !== null) throw new Error(`Development root must be nonterminal:${label}`);
  return true;
}
...
function verifyAuthorization(auth) {
  if (auth.studyId !== "REEOE-STUDY1" || auth.stageId !== "REEOE-S1-DEVELOPMENT-2026-08-28-v1") {
    throw new Error("Unexpected Stage 1 authorization identity");
  }
  if (auth.executionAuthorized !== true || auth.scientificInferenceAuthorized !== false
    ...
    throw new Error("Invalid Stage 1 authorization flags");
  }
  for (const [relative, expected] of Object.entries(auth.sourceGitBlobSha)) {
    const observed = gitBlob(relative);
    if (observed !== expected) throw new Error(`Stage 1 source freeze mismatch ${relative}: ${observed} != ${expected}`);
  }
}
...
function selectedRootRow(root) {
  return {
    ...
    exactLegalMoveCount: root.legalMoveCount,
  };
}
...
function main() {
  ...
  if (spec.studyId !== "REEOE-STUDY1" || spec.stageId !== "REEOE-S1-DEVELOPMENT-2026-08-28-v1") {
    throw new Error("Unexpected Stage 1 spec identity");
  }
  ...
  const eligible = allRoots
    .filter((root) => root.nonEmptyPitCount <= envelope.maximumNonEmptyPitCount
      && root.legalMoveCount <= envelope.maximumExactLegalMoveCount)
  ...
  const developmentRows = selected.map((root) => {
    const closure = S.enumerateClosure([root.state], closureOptions);
    return {
      ...selectedRootRow(root),
      closure: normalizeClosure(closure),
    };
  });

  const completeCount = developmentRows.filter((row) => row.closure.complete).length;
  const productionAcceptance = selected.length >= spec.stage1AcceptanceRule.minimumSelectedRoots
  ...
  if (sha256Stable(reopened.development) !== result.developmentCoreSha256) throw new Error("Stage 1 reopen/hash mismatch");
  if (reopened.scientificInferenceAuthorized !== false || reopened.formalExactDecisionAuthorized !== false
    || reopened.stage2ExecutionAuthorized !== false || reopened.forbiddenOutcomeInspectionPerformed !== false) {
    throw new Error("Stage 1 output authorization boundary failure");
  }

  console.log(JSON.stringify({
    outputPath,
    productionDevelopmentStatus: result.productionDevelopmentStatus,
    uniqueWitnessRootCount: allRoots.length,
    eligibleRootCount: eligible.length,
    selectedRootCount: selected.length,
    completeClosureCount: completeCount,
    selectedRoots: developmentRows.map((row) => ({
      seed: row.seed,
      ply: row.ply,
      rootStateKey: row.rootStateKey,
      nonEmptyPitCount: row.nonEmptyPitCount,
      exactLegalMoveCount: row.exactLegalMoveCount,
      closure: row.closure,
    })),
    allEncounteredRootSetSha256: developmentCore.allEncounteredRootSetSha256,
    developmentCoreSha256: result.developmentCoreSha256,
  }, null, 2));
}
