...
const SPEC = JSON.parse(fs.readFileSync(path.join(ROOT, "doc/state-space-game-tree-growth-estimation/preregistration/STAGE_0_TECHNICAL_SPEC.json"), "utf8"));
...
function main() {
  const production = JSON.parse(fs.readFileSync(path.join(OUT_DIR, "stage0-production-result.json"), "utf8"));
  ...
  ensure(production.realDevelopmentCandidateEvaluationPerformed === false, "Stage 0 development evaluation leakage detected");
  ...
  const snapshot = { cumulativeRawStates: 1, depthLabelledEdges: 1, parentExpansions: 1, moveEvaluations: 1, cumulativeTreeNodeOccurrences: 1, residentSetBytes: 1, artifactBytes: 1, wallClockSeconds: 1 };
  ...
  {
    const corruptRows = [{ depth: 0, syntheticCount: 1 }, { depth: 2, syntheticCount: 2 }, { depth: 3, syntheticCount: 3 }, { depth: 4, syntheticCount: 4 }, { depth: 5, syntheticCount: 5 }];
    controls.depthGap = expectFailure("depth gap", () => ind.predict(corruptRows, "syntheticCount", 6, ind.CANDIDATES[0]));
  }
  {
    const bad = JSON.parse(JSON.stringify(production.syntheticEstimatorChecks.checks[0]));
    bad.prediction *= 1.01;
    controls.numericMismatch = !nearlyEqual(bad.prediction, synthetic.checks[0].prediction, tolerance);
  ...
  ensure(passed, "Stage 0 negative control failure");
  const resultCore = {
    schemaVersion: 1,
  ...
  fs.writeFileSync(path.join(OUT_DIR, "STAGE_0_TECHNICAL_RESULT.json"), `${JSON.stringify(finalResult, null, 2)}\n`, "utf8");
  console.log(`SSGTGE_STAGE0_INDEPENDENT=${JSON.stringify({ passed: true, resultCoreSha256: finalResult.resultCoreSha256 })}`);
}
