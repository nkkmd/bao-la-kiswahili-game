...
function parseArgs(argv) {
  const options = {
    ...
    lgGames: null,
    lgCandidates: null,
    output: "artifacts/local/phase-transition-d3-reversal-replication-v1/paired-game-endpoints.json",
  };
...
function buildPairs(p2Games, p2Candidates, lgGames, lgCandidates, minimumPliesRemaining) {
  ...
  if (E020.canonicalJson(p2Seeds) !== E020.canonicalJson(lgSeeds)) throw new Error("P2 and LG game seed sets differ");
  return p2Seeds.map((seed) => {
    const left = p2.get(seed);
    const right = lg.get(seed);
    if (left.gameIndex !== right.gameIndex) throw new Error(`Paired gameIndex mismatch for seed ${seed}`);
    return {
      seed,
...
module.exports = { buildPairs, eligibleCandidate, endpointsBySeed, parseArgs, run };
