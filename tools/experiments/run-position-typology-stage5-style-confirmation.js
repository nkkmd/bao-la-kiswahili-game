...
function loadSpec() {
  ...
  if (spec.formalExperiment !== true || spec.exploratory !== false) {
    throw new Error("Stage 5 preregistration boundary mismatch");
  }
  if (spec.stage !== "stage5-playing-style-continuous-independent-confirmation") {
    throw new Error(`Unexpected preregistration stage: ${spec.stage}`);
  }
  if (spec.preregistrationId !== "PTYP-S5-STYLE-CONTINUOUS-CONFIRM-2026-08-10-v1") {
  ...
  if (spec.corpus.gamesPerCondition * spec.corpus.conditions.length !== spec.corpus.games) {
    throw new Error("gamesPerCondition mismatch");
  }
  return spec;
}
...
function parseArgs(argv) {
  const options = {
    output: "artifacts/local/position-typology/stage5-playing-style-confirmation-v1",
    force: false,
    status: false,
  };
...
function formalConfig(spec, sourceHashes) {
  return {
    ...
    exploratory: false,
    coordinateDefinitionHash: spec.coordinateDefinition.requiredHash,
    preregistrationId: spec.preregistrationId,
    preregistrationSpecFileSha256: sourceHashes[
      "doc/position-typology/preregistration/STAGE_5_PLAYING_STYLE_CONFIRMATION_SPEC.json"
    ],
...
function runGame(config, gameIndex) {
  ...
  const gameId = `ptyp-s5c-${String(gameIndex).padStart(4, "0")}`;
  const observations = [];
  const moves = [];
...
function main() {
  ...
  if (source.sourceTreeDirty) {
    throw new Error("Formal Stage 5 confirmation requires a clean source tree");
  }
  ...
  for (let gameIndex = 0; gameIndex < config.games; gameIndex += 1) {
    ...
    games.push(game);
    console.log(`[position-typology stage5 style confirmation] ${gameIndex + 1}/${config.games} ${game.gameId}`);
  }
  ...
  const manifest = {
    ...
    status: "stage5-playing-style-confirmation-corpus-complete",
    formalExperiment: true,
    exploratory: false,
    confirmationExperiment: true,
    coordinateDefinitionHash: spec.coordinateDefinition.requiredHash,
    preregistrationId: spec.preregistrationId,
    preregistrationSpecFileSha256: source.sourceFileSha256[
      "doc/position-typology/preregistration/STAGE_5_PLAYING_STYLE_CONFIRMATION_SPEC.json"
    ],
    heldOutSeedBlock: {
