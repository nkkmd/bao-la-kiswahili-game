#!/usr/bin/env node
"use strict";
const fs=require("node:fs"),path=require("node:path"),childProcess=require("node:child_process"),crypto=require("node:crypto");
const ROOT=path.resolve(__dirname,"../.."),DOC=path.join(ROOT,"doc/continuous-local-geometry-representation");
function need(x,m){if(!x)throw new Error(m);}function json(p){return JSON.parse(fs.readFileSync(p,"utf8"));}function git(...a){return childProcess.execFileSync("git",a,{cwd:ROOT,encoding:"utf8"}).trim();}function canon(v){if(v===null||typeof v!=="object")return JSON.stringify(v);if(Array.isArray(v))return`[${v.map(canon).join(",")}]`;return`{${Object.keys(v).sort().map(k=>`${JSON.stringify(k)}:${canon(v[k])}`).join(",")}}`;}function sha(x){return crypto.createHash("sha256").update(typeof x==="string"?x:canon(x),"utf8").digest("hex");}
...
const stage1Prod=fs.readFileSync(path.join(ROOT,"tools/experiments/lib/clgr-stage1-production.js"),"utf8"),stage1Ind=fs.readFileSync(path.join(ROOT,"tools/experiments/lib/clgr-stage1-independent.js"),"utf8"),clgrProd=fs.readFileSync(path.join(ROOT,"tools/experiments/lib/clgr-production.js"),"utf8"),clgrInd=fs.readFileSync(path.join(ROOT,"tools/experiments/lib/clgr-independent.js"),"utf8"),runner=fs.readFileSync(path.join(ROOT,"tools/experiments/run-clgr-stage1-development.js"),"utf8"),workflow=fs.readFileSync(path.join(ROOT,".github/workflows/clgr-stage1-development-v1.yml"),"utf8");
need(stage1Prod.includes("./clgr-production.js")&&!stage1Prod.includes("clgr-independent.js")&&!stage1Prod.includes("lgtgmiv-stage1-independent.js"),"Stage1 production aliases independent");
need(stage1Ind.includes("./clgr-independent.js")&&!stage1Ind.includes("clgr-production.js")&&!stage1Ind.includes("lgtgmiv-stage1-production.js"),"Stage1 independent aliases production");
need(clgrProd.includes("./lgtgmiv-stage1-production.js")&&!clgrProd.includes("lgtgmiv-stage1-independent.js"),"CLGR production upstream separation invalid");
need(clgrInd.includes("./lgtgmiv-stage1-independent.js")&&!clgrInd.includes("lgtgmiv-stage1-production.js"),"CLGR independent upstream separation invalid");
