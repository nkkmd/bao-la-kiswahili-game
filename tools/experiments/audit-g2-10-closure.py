...
FINAL_TOKEN = "STAGE1-DEVELOPMENT-BLOCKED-NO-REPRESENTATION"
STAGE2_TOKEN = "NOT-AUTHORIZED-NOT-EXECUTED"
...
require(stage1["disposition"] == FINAL_TOKEN, "Stage 1 disposition mismatch")
...
require(final["formalStudyDecision"] == FINAL_TOKEN, "final Study decision mismatch")
...
require(final["stages"]["stage2"]["disposition"] == STAGE2_TOKEN, "Stage 2 disposition mismatch")
...
for rel in [
    ...
    require(FINAL_TOKEN in text, f"final token missing from {rel}")
    require(STAGE2_TOKEN in text, f"Stage 2 token missing from {rel}")
...
for name, text in [("README.md", root_readme), ("RESEARCH_INDEX.md", index), ("FUTURE_RESEARCH_AGENDA.md", agenda)]:
    ...
    require(FINAL_TOKEN in text, f"final token missing from {name}")
