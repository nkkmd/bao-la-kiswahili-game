...
const Runner = require("./run-phase-transition-search-profile-generalization.js");
const PairBuilder = require("./build-phase-transition-search-profile-generalization-pairs.js");
const Evaluator = require("./evaluate-phase-transition-search-profile-generalization.js");
const Structure = require("./summarize-phase-transition-search-profile-generalization-structure.js");

function parseArgs(argv) {
  const options = {
    phase: "status",
    lock: "artifacts/phase-transition/search-profile-generalization-v2/execution-lock.json",
    policy: "config/experiments/phase-transition-search-profile-generalization-execution-policy-v2.json",
    approvalToken: null,
  };
...
function assertLockedInputs(lock, policyPath, policy, preregistration, repositoryRoot) {
  ...
  if (path.relative(repositoryRoot, resolvedPolicyPath) !== lock.executionPolicy.path) errors.push("Execution policy path differs from the execution lock.");
  if (sha256(fs.readFileSync(resolvedPolicyPath)) !== lock.executionPolicy.sha256) errors.push("Execution policy hash differs from the execution lock.");
  ...
  if (!fs.existsSync(preregistrationPath)) errors.push("Preregistration file is missing.");
    ...
    if (path.relative(repositoryRoot, preregistrationPath) !== lock.preregistration.path) errors.push("Preregistration path differs from the execution lock.");
    if (sha256(fs.readFileSync(preregistrationPath)) !== lock.preregistration.sha256) errors.push("Preregistration hash differs from the execution lock.");
  }
  for (const [name, value] of [
    ["corpus", preregistration.corpus],
    ["primaryEndpoint", preregistration.primaryEndpoint],
    ["conditionDecisionRule", preregistration.conditionDecisionRule],
    ["globalDecisionRule", preregistration.globalDecisionRule],
    ["individualStandaloneInference", preregistration.individualStandaloneInference],
    ["structuralSecondaryEndpoint", preregistration.structuralSecondaryEndpoint],
  ]) {
    if (E019.canonicalJson(lock[name]) !== E019.canonicalJson(value)) errors.push(`Preregistered ${name} differs from the execution lock.`);
  }
...
function assertLockedEnvironment(lock, policy, repositoryRoot) {
  const errors = [];
  if (process.env.GITHUB_ACTIONS === "true") errors.push("Formal E-019 run is forbidden in GitHub Actions.");
  ...
  if (lock.status !== "prepared-approved" || lock.approval?.approved !== true) errors.push(`Execution lock is not an approved E-019 lock: ${lock.status}.`);
...
function approve(lock, policy, suppliedToken) {
  if (policy.formalExecutionAllowed !== true || policy.status !== "approved-awaiting-local-lock") throw new Error("Formal E-019 execution is disabled by the repository execution policy.");
  if (!suppliedToken || suppliedToken !== policy.approvalToken) throw new Error("Exact E-019 formal-run approval token is required.");
  if (sha256(suppliedToken) !== lock.approval.approvalTokenSha256) throw new Error("Approval token does not match the E-019 execution lock.");
  if (lock.approval?.approved !== true) throw new Error("E-019 execution lock was not prepared after authorization.");
}
...
function status(lock, policy, preregistration) {
  const conditions = {};
  for (const condition of E019.allConditions(preregistration)) {
    ...
    conditions[condition.conditionId] = {
      ...
      plannedGames: condition.pairedSeeds,
...
function plannedGamesForCondition(preregistration, conditionId) {
  return E019.conditionById(preregistration, conditionId).pairedSeeds;
}
...
function requireConditionComplete(repositoryRoot, policy, preregistration, conditionId) {
  ...
  const planned = plannedGamesForCondition(preregistration, conditionId);
  if (Number(manifest.completedGames) !== planned || games.length !== planned) throw new Error(`${conditionId}: formal corpus is not complete.`);
...
function runCorpus(lock, policy, preregistration, options) {
  ...
  const loaded = E019.loadPreregistration(path.resolve(lock.environment.repositoryRoot, policy.paths.preregistration));
  const runOptions = { output: path.resolve(lock.environment.repositoryRoot, policy.paths.corpusRoot), fixtureGames: null, fixtureBaseSeed: null, force: false, status: false };
  const results = [];
  for (const condition of E019.allConditions(preregistration)) results.push(Runner.runCondition(loaded, condition.conditionId, runOptions, "formal"));
  return results;
}
...
function analyzeCorpus(lock, policy, preregistration) {
  ...
  for (const condition of E019.allConditions(preregistration)) {
    ...
    execute(policy.pythonCommand, ["tools/experiments/analyze-phase-transition-archetypes.py", "--input", corpusRoot, "--output", outputs.archetypes], repositoryRoot);
...
function verifyCorpus(lock, policy, preregistration, lockPath) {
  for (const condition of E019.allConditions(preregistration)) requireConditionComplete(lock.environment.repositoryRoot, policy, preregistration, condition.conditionId);
  execute(process.execPath, [
...
function assertIntegrity(repositoryRoot, policy) {
  const filePath = path.resolve(repositoryRoot, policy.paths.integrityRoot, "search-profile-generalization-integrity.json");
  if (!fs.existsSync(filePath)) throw new Error("Formal E-019 integrity result is missing; run verify first.");
  const result = readJson(filePath);
  if (result.valid !== true || result.mode !== "formal" || result.experimentId !== "E-019") throw new Error("Formal E-019 integrity verification has not passed.");
  return result;
}
...
function evaluateCorpus(lock, policy, preregistration) {
  const repositoryRoot = lock.environment.repositoryRoot;
  for (const condition of E019.allConditions(preregistration)) requireConditionComplete(repositoryRoot, policy, preregistration, condition.conditionId);
  ...
  try {
    const built = PairBuilder.run({
      ...
      corpusRoot: path.resolve(repositoryRoot, policy.paths.corpusRoot),
      analysisRoot: path.resolve(repositoryRoot, policy.paths.analysisRoot),
    ...
    const result = Evaluator.evaluate(preregistration, built.strata);
    const output = path.resolve(repositoryRoot, policy.paths.evaluationRoot);
    fs.mkdirSync(output, { recursive: true });
    fs.writeFileSync(path.join(output, "search-profile-generalization-result.json"), `${JSON.stringify(result, null, 2)}\n`);
...
function main(options) {
  ...
  E019.validatePreregistration(preregistration);
