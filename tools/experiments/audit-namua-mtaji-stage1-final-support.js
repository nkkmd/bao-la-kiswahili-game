...
function parseArgs(argv) {
  const options = {
    ...
    finalExtension: "artifacts/local/namua-mtaji-transition/stage1-final-extension-v1",
    output: null,
  };
  for (let index = 0; index < argv.length; index += 2) {
    ...
    else if (key === "--final-extension") options.finalExtension = value;
    else if (key === "--output") options.output = value;
    else throw new Error(`Unknown argument: ${key}`);
  }
  options.output ||= path.join(options.finalExtension, "stage1-final-support-audit.json");
  return options;
}
...
function validateFinalManifest(directory) {
  ...
  if (manifest.formalExperiment !== false
    ...
    throw new Error("Final extension scientific boundary mismatch");
  }
  const policy = manifest.config?.extensionPolicy || {};
  if (policy.extensionOrdinal !== 2
    || policy.finalExposureDrivenExtension !== true
    || policy.furtherExposureDrivenExtensionAfterThisAllowed !== false
    || policy.stage2ReadinessMinimumUniqueExposureUnits !== MIN_UNIQUE_EXPOSURE_UNITS
    || policy.stage2ReadinessMinimumUniqueExposureTrajectories !== MIN_UNIQUE_EXPOSURE_TRAJECTORIES) {
    throw new Error("Final extension stopping/readiness policy mismatch");
  }
  return manifest;
}
...
function main() {
  ...
  const finalExtension = path.resolve(options.finalExtension);
  const finalManifest = validateFinalManifest(finalExtension);

  const rows = [
    ...cbeRows(pilot, "primary-pilot"),
    ...cbeRows(extension, "extension-1"),
    ...cbeRows(finalExtension, "final-extension"),
  ];
  ...
  const report = {
    ...
    disposition: readiness
      ? "proceed-to-stage2-design-freeze-after-final-riskset-review"
      : "do-not-add-more-exposure-targeted-stage1-sampling-under-current-frozen-design",
    exposures: unique,
    interpretationBoundary: {
      ...
      readinessThresholdRelaxationAuthorized: false,
      furtherExposureDrivenSamplingIfGateFailsAuthorized: false,
    },
  };
