...
const AUTH = path.join(ROOT, "doc/search-instability-local-geometry-mechanism/authorizations/STAGE_0_TECHNICAL_AUTHORIZATION_V4.json");
const TRIGGER = path.join(ROOT, "doc/search-instability-local-geometry-mechanism/authorizations/silgm-stage0-v4-trigger.txt");
...
function main(){
  ...
  need(a.stageId==="SILGM-S0-TECHNICAL-2026-09-03-v4","stage mismatch");
  ...
  need(parent===a.expectedToolingCommit,"trigger parent is not frozen v4 tooling commit");
  ...
  need(!prod.includes("31710001")&&!prod.includes("31720001")&&!indep.includes("31710001")&&!indep.includes("31720001"),"fresh scientific seed literal present in Stage0 libraries");
  ...
  console.log(JSON.stringify({authorizationVerified:true,studyId:a.studyId,stageId:a.stageId,head,parent,authorizationNonce:a.authorizationNonce,technicalMtajiCandidateRange:"31709002..31709008",rawWrapperFreshSeedLiteralCheck:"DELEGATED-TO-MATERIALIZATION-VERIFY-ONLY",freshScientificSeedAccessAuthorized:false,protectedDepth10AccessAuthorized:false}));
}
main();
