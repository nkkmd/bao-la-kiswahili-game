...
function houseFixture(){const s=E.initialState();s.pits=[[Array(8).fill(0),Array(8).fill(0)],[Array(8).fill(0),Array(8).fill(0)]];s.pits[0][E.FRONT][2]=1;s.pits[0][E.FRONT][E.HOUSE]=6;s.pits[0][E.FRONT][6]=1;s.pits[1][E.FRONT][5]=5;s.pits[1][E.FRONT][6]=1;s.reserve=[10,10];s.houseOwned=[true,true];s.player=0;s.phase="namua";s.winner=null;s.reason="";s.turn=10;s.pending=[0,0];return s;}
...
function main(){const output=process.argv[2]||"doc/bao-rule-mechanism-geometry-intervention/results/stage-0/STAGE_0_TECHNICAL_RESULT.json";
  const initial=E.initialState();
  const prodInitial=P.measureState(E,initial,"T7-INITIAL"),indInitial=I.measureState(E,initial,"T7-INITIAL");
  ...
  const gates={
    ...
    t3NyumbaPair:pPairs.length>0&&same(pPairs.map(x=>x.physicalMoveKey),iPairs.map(x=>x.physicalMoveKey)),
