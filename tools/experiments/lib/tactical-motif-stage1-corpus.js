...
const SpecValidator = require("../validate-tactical-motif-stage1-spec.js");

const ROOT = path.resolve(__dirname, "../../..");
const SPEC_PATH = path.join(ROOT, "doc/tactical-motifs/preregistration/STAGE_1_EXPLORATORY_SPEC.json");
const AUTH_PATH = path.join(ROOT, "doc/tactical-motifs/preregistration/STAGE_1_EXPLORATORY_AUTHORIZATION.json");
const DEFAULT_OUTPUT = path.join(ROOT, "artifacts/local/tactical-motifs/stage1-exploratory-v1");
...
function loadAuthorization(specSha256) {
  ...
  if (auth.schemaVersion !== 1 || auth.stageId !== "TM-S1-EXPLORATORY-2026-08-14-v1"
    || auth.stage1GenerationAuthorized !== true || auth.scientificInferenceAuthorized !== false
    || auth.confirmatoryReuseAllowed !== false || auth.specSha256 !== specSha256) {
...
function runGame(spec, specSha256, gameIndex) {
  ...
  const gameId = `tm-s1-${String(gameIndex).padStart(4, "0")}`;
  let state = E.initialState();
  const observations = [];
  const moves = [];
  for (let ply = 0; ply <= spec.population.maxPly; ply += 1) {
    const observation = extractPositionTypologyObservation(state, { gameId, conditionId: condition.id, seed, ply });
    ...
    moves.push({
      ply, player: state.player, source, conditionId: condition.id,
      move: JSON.parse(JSON.stringify(move)), moveKey: AI.moveKey(move),
  ...
  return {
    ...
    confirmatoryReuseAllowed: false, gameId, gameIndex, seed, conditionId: condition.id,
    observations, moves, openingPrefix: openingPrefixIdentity(moves, spec),
    historicalTrajectoryHash, ruleTrajectoryHash, winner: state.winner,
...
function stateFromObservation(observation) {
  return {
    ...
    reserve: [...observation.state.reserve], houseOwned: [...observation.state.houseOwned],
    player: observation.player, phase: observation.phase, winner: observation.winner,
    reason: observation.reason || "", turn: observation.turn, pending: [...observation.state.pending],
  };
}
...
module.exports = {
  AUTH_PATH, DEFAULT_OUTPUT, ROOT, SOURCE_FILES, SPEC_PATH,
  aiMove, assignedPhase, conditionForGame, gamePath, loadAuthorization, loadSpec,
  measurementPath, openingPrefixIdentity, provenance, readGames, readJson,
  representativeGames, runGame, selectionRank, sha256, sourceFileSha256,
  stateFromObservation, writeJson,
};
