...
function digest(x){return crypto.createHash("sha256").update(x,"utf8").digest("hex");}
function cellForSeed(seed){
  assertOk(Number.isInteger(seed)&&seed>=0,"seed must be nonnegative integer");
  return copy(CELLS[seed % 4]);
}
function describe(replay,row){
  if(!row)return null;
  return {
    phase:row.phase,
    ...
    openingPrefixSha256:digest(replay.moveKeys.slice(0,16).join("\n")),
...
function replayAssigned(engine,seed,maxPly=80){
  ...
  return {
    ...
    namua:describe(replay,anchors.namua),
    mtaji:describe(replay,anchors.mtaji),
    pairComplete:Boolean(anchors.complete)
  };
}
function selectionKey(source){
  assertOk(source&&source.pairComplete&&source.namua&&source.mtaji,"complete source pair required");
  return digest([
    "CTGTS-S1",
    source.cellId,
    source.seed,
...
module.exports={CELLS,cellForSeed,replayAssigned,selectionKey,digest};
