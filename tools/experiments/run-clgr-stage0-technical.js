...
need(SPEC.technicalSeedStart===31909001&&SPEC.technicalSeedEnd===31909008,"technical seed mismatch");
...
function syntheticRep(id,offset){const axes={};for(let i=0;i<P.AXES.length;i++)axes[P.AXES[i]]=P.rational(BigInt(i+1+offset),BigInt(i+2));return P.represent({rootRawSha256:id,axes});}
function syntheticRepI(id,offset){const axes=Object.create(null);for(let i=0;i<I.AXES.length;i++)axes[I.AXES[i]]=I.rational(BigInt(i+1+offset),BigInt(i+2));return I.represent({rootRawSha256:id,axes});}
const repsP=[syntheticRep("r0",0),syntheticRep("r1",1),syntheticRep("r2",2),syntheticRep("r3",3),syntheticRep("r4",4)];
const repsI=[syntheticRepI("r0",0),syntheticRepI("r1",1),syntheticRepI("r2",2),syntheticRepI("r3",3),syntheticRepI("r4",4)];
same(P.distanceRows(repsP),I.distanceRows(repsI),"synthetic distance matrix mismatch");
same(P.neighbors(repsP,3),I.neighbors(repsI,3),"synthetic neighborhood mismatch");
const permuted=[repsP[4],repsP[1],repsP[3],repsP[0],repsP[2]];
same(P.distanceRows(repsP),P.distanceRows(permuted),"root-order distance invariance failed");same(P.neighbors(repsP,3),P.neighbors(permuted,3),"root-order neighborhood invariance failed");
...
let relayP=false,relayI=false;try{P.measureRoot(fakeEngine,{phase:"namua",sourceSeed:31909001,selectedPly:1,rootState:clone(fakeRoot)});}catch(e){relayP=/relay-limit enumeration/.test(String(e&&e.message));}
try{I.measureRoot(fakeEngine,{phase:"namua",sourceSeed:31909001,selectedPly:1,rootState:clone(fakeRoot)});}catch(e){relayI=/relay-limit enumeration/.test(String(e&&e.message));}
need(relayP&&relayI,"relay-limit fail-closed sentinel not exercised");
...
const technicalRepsP=measuredP.map(x=>x.representation),technicalRepsI=measuredI.map(x=>x.representation);need(technicalRepsP.length===2,"technical root count mismatch");same(P.distanceRows(technicalRepsP),I.distanceRows(technicalRepsI),"technical exact distance mismatch");
...
const core={schemaVersion:1,studyId:"CLGR-STUDY1",stageId:SPEC.stageId,evidenceClass:"TECHNICAL-FIXTURE",representationId:P.REPRESENTATION_ID,axisUniverse:P.AXES,syntheticAxisExact:true,syntheticTransformExact:true,exactRationalArithmetic:true,exactL1Distance:true,k3TieInclusiveNeighborhood:true,rootOrderInvariance:true,relayLimitFailClosed:true,technicalSeed:selected.seed,technicalSeedScientificUseProhibited:true,technicalRoots:selected.roots.map(x=>({phase:x.phase,selectedPly:x.selectedPly,rootRawSha256:x.rootRawSha256})),productionIndependentDepth5Exact:true,productionIndependentAxisExact:true,productionIndependentCoordinateExact:true,productionIndependentDistanceExact:true,implementationSeparation:true,technicalTimings:timings,maxCombinedRootMs:Math.max(...timings.map(x=>x.combinedMs)),freshScientificSeedAccess:false,stage1SeedAccess:false,stage2SeedAccess:false,protectedDepth10Access:false};
const result={...core,deterministicCoreSha256:sha(core),stageDisposition:"STAGE0-PASS"};
const arg=process.argv.indexOf("--output"),out=arg>=0?process.argv[arg+1]:path.join(DOC,"results/stage-0-v1/STAGE_0_TECHNICAL_RESULT.json");fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(result,null,2)+"\n");console.log(`CLGR_STAGE0_RESULT=${JSON.stringify(result)}`);
