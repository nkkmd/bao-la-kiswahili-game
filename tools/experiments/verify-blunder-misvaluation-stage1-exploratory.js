...
function verifyGame(record, gameIndex, spec, specSha256, recomputeSearch = true) {
  if (record.specSha256 !== specSha256 || record.gameIndex !== gameIndex) {
    throw new Error(`Game identity mismatch: ${gameIndex}`);
  }
  ...
  if (record.seed !== seed || record.conditionId !== condition.id) {
    throw new Error(`Seed/condition mismatch: ${gameIndex}`);
  }
  const expectedGameId = `bmp-s1-${String(gameIndex).padStart(4, "0")}`;
  if (record.gameId !== expectedGameId) throw new Error(`gameId mismatch: ${gameIndex}`);
  ...
  for (let ply = 0; ply <= spec.population.maxPly; ply += 1) {
    ...
    if (!storedObservation || stableStringify(observation) !== stableStringify(storedObservation)) {
      throw new Error(`Observation mismatch game ${gameIndex} ply ${ply}`);
    }
    if (state.winner !== null || ply === spec.population.maxPly) break;
    const stored = record.moves[moves.length];
    if (!stored || stored.ply !== ply) throw new Error(`Missing move game ${gameIndex} ply ${ply}`);
    let move;
    let search = null;
    if (ply < spec.population.opening.plies) {
      ...
      if (stored.source !== "opening-random") {
        throw new Error(`Opening source mismatch game ${gameIndex} ply ${ply}`);
      }
      ...
      if (stored.source !== "trajectory-ai") {
        throw new Error(`AI source mismatch game ${gameIndex} ply ${ply}`);
      }
      sameSearch(search, stored.generationSearch);
    } else {
      const legal = E.moveVariants(state);
      move = legal.find((candidate) => AI.moveKey(candidate) === stored.moveKey);
    ...
    if (AI.moveKey(move) !== stored.moveKey || stableStringify(move) !== stableStringify(stored.move)) {
      throw new Error(`Move mismatch game ${gameIndex} ply ${ply}`);
    }
    if (stored.beforeHistoricalStateHash !== observation.identity.historicalStateHash
      ...
      throw new Error(`Before identity mismatch game ${gameIndex} ply ${ply}`);
    }
    ...
    if (stored.afterHistoricalStateHash !== after.historicalStateHash
      ...
      throw new Error(`After identity mismatch game ${gameIndex} ply ${ply}`);
    }
    moves.push({ moveKey: stored.moveKey });
    state = applied.state;
  }
  ...
  if (historicalTrajectoryHash !== record.historicalTrajectoryHash
    ...
    throw new Error(`Trajectory/opening hash mismatch game ${gameIndex}`);
  }
  if (record.plies !== record.moves.length || record.observations.length !== observations.length) {
    throw new Error(`Length mismatch game ${gameIndex}`);
  }
  return {
    gameIndex,
    seed,
...
function main() {
  ...
  C.loadAuthorization(specSha256);
  const manifest = C.readJson(path.join(o.output, "manifest.json"));
  if (manifest.specSha256 !== specSha256 || manifest.summary.games !== spec.population.games) {
    throw new Error("Manifest/spec mismatch");
  }
  const verified = [];
  for (let i = 0; i < spec.population.games; i += 1) {
    const file = C.gamePath(o.output, i);
    if (!fs.existsSync(file)) throw new Error(`Missing game ${i}`);
    verified.push(verifyGame(C.readJson(file), i, spec, specSha256, o.recomputeSearch));
    console.error(`[bmp stage1 verify] ${i + 1}/${spec.population.games}`);
  }
  const conditionCounts = verified.reduce((counts, item) => {
    counts[item.conditionId] = (counts[item.conditionId] || 0) + 1;
    return counts;
  }, {});
  const result = {
    schemaVersion: 1,
    ...
    specSha256,
    passed: true,
    fullSearchRecomputation: o.recomputeSearch,
  ...
  C.writeJson(path.join(o.output, "verification.json"), result);
