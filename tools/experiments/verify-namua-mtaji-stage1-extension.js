...
const { sourceFileHashes } = require("./run-namua-mtaji-stage1-extension.js");
...
function main() {
  ...
  if (manifest.formalExperiment !== false
    ...
    throw new Error("Stage 1 extension scientific boundary mismatch");
  }
  if (manifest.config.stage !== "stage1-exposure-support-extension") {
    throw new Error(`Unexpected extension identity: ${manifest.config.stage}`);
  }
  ...
  if (!sourceHashesMatch) throw new Error("Current source-file hashes differ from extension provenance");

  const report = {
    schemaVersion: 1,
    generatedAt: new Date().toISOString(),
    status: "stage1-exposure-support-extension-verification-complete",
    passed: true,
