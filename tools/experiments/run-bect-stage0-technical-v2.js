...
const REFREEZE=JSON.parse(fs.readFileSync(path.join(DOC,"prereg/STAGE_0_V2_TECHNICAL_REFREEZE.json"),"utf8"));
const TECHNICAL_SEED=31500001;
const STAGE_ID="BECT-S0-TECHNICAL-2026-09-02-v2";
...
need(REFREEZE.stageId===STAGE_ID&&REFREEZE.sameV1RerunAllowed===false,"v2 refreeze mismatch");
need(REFREEZE.technicalSeed===TECHNICAL_SEED,"technical seed mismatch");
need(TECHNICAL_SEED<SPEC.stages[1].seedStart,"technical seed collides with scientific namespace");
...
need(!util.isDeepStrictEqual(fp.levels,fi.levels),"prototype-sensitive negative control not exercised");
...
e=firstEvent(classify([2,1,2,1]));need(e&&e.direction==="UP"&&e.disposition==="REVERSAL","reversal/recovery fixture failed");
e=firstEvent(classify([2,1,2,2]));need(e&&e.direction==="UP"&&e.disposition==="STALL","stall fixture failed");
need(firstEvent(classify([2,1,2,3],["namua","namua","mtaji","mtaji"]))===null,"cross-phase primary event not excluded");

const reopenLow=fixture(),reopenHigh=fixture();for(const x of reopenLow.reconstructionCore.parentLayers)x.branchReopeningCount=0;for(const x of reopenHigh.reconstructionCore.parentLayers)x.branchReopeningCount=1;
need(P.sign(P.subtract(P.deriveLevel(reopenHigh).levels["BECT-M7-BRANCH-REOPENING-FRACTION"],P.deriveLevel(reopenLow).levels["BECT-M7-BRANCH-REOPENING-FRACTION"]))==="UP","reopening fixture failed");
const extinctLow=fixture(),extinctHigh=fixture();for(const x of extinctLow.reconstructionCore.parentLayers)x.branchExtinctionCount=0;for(const x of extinctHigh.reconstructionCore.parentLayers)x.branchExtinctionCount=2;
need(P.sign(P.subtract(P.deriveLevel(extinctHigh).levels["BECT-M8-BRANCH-EXTINCTION-FRACTION"],P.deriveLevel(extinctLow).levels["BECT-M8-BRANCH-EXTINCTION-FRACTION"]))==="UP","extinction fixture failed");

const pp=P.replay(E,TECHNICAL_SEED,REFREEZE.authorizedCorrection.replayMaximumPly),ii=I.replay(E,TECHNICAL_SEED,REFREEZE.authorizedCorrection.replayMaximumPly);
same({path:pp.path,roots:pp.rows.map(x=>({ply:x.ply,moveKey:x.moveKey,parent:x.parentRawSha256,root:x.root.rootRawSha256,phase:x.root.phase,winner:x.root.rootState.winner}))},{path:ii.path,roots:ii.rows.map(x=>({ply:x.ply,moveKey:x.moveKey,parent:x.parentRawSha256,root:x.root.rootRawSha256,phase:x.root.phase,winner:x.root.rootState.winner}))},"technical trajectory replay mismatch");
need(pp.rows.length>=2,"technical trajectory lacks adjacent roots");
...
let pair=null;
for(let k=pp.rows.length-2;k>=0;k--){const a=pp.rows[k],b=pp.rows[k+1];if(a.root.rootState.winner===null&&b.root.rootState.winner===null&&b.ply===a.ply+1){pair=[a,b];break;}}
need(pair!==null,"no consecutive nonterminal technical root pair available");
const techRoots=pair.map(x=>x.root);
...
for(let k=0;k<2;k++){
  ...
  same(pm[k].upstreamFamilyCoreSha256,im[k].upstreamFamilyCoreSha256,"technical upstream family mismatch");
  same(pm[k].bect,im[k].bect,"technical BECT level mismatch");
}
need(pair[1].parentRawSha256===pm[0].source.rootRawSha256,"overlapping-window successor semantics failed");
const orderedSeries=pm.map((m,k)=>({ply:pair[k].ply,phase:m.source.phase,levels:m.bect.levels,rootRawSha256:m.source.rootRawSha256}));
need(orderedSeries.length===2&&orderedSeries[1].ply===orderedSeries[0].ply+1,"time-indexed overlapping pair lost");
...
const insertionVariant=clone(fp);insertionVariant.levels={};for(const id of P.METRICS.slice().reverse())insertionVariant.levels[id]=fp.levels[id];need(sha(fp)===sha(insertionVariant),"canonical property-order invariance failed");
...
need(psrc.includes('lgtgmiv-stage1-production.js')&&!psrc.includes('lgtgmiv-stage1-independent.js'),"production separation failed");
need(isrc.includes('lgtgmiv-stage1-independent.js')&&!isrc.includes('lgtgmiv-stage1-production.js')&&!isrc.includes('bect-production.js'),"independent separation failed");

const technicalCore={schemaVersion:1,studyId:"BECT-STUDY1",stageId:STAGE_ID,evidenceClass:"TECHNICAL-FIXTURE",priorStage0V1Disposition:"TECHNICAL-INVALID",syntheticLevelExact:true,eventGrammarExact:true,noChangeControl:true,expansionControl:true,compressionControl:true,persistenceControl:true,reversalRecoveryControl:true,stallControl:true,crossPhaseExclusionControl:true,reopeningControl:true,extinctionControl:true,canonicalPrototypeInsensitiveEquality:true,prototypeSensitiveEqualityNegativeControl:true,propertyOrderInvariance:true,technicalTrajectorySeed:TECHNICAL_SEED,technicalTrajectorySeedPermanentlyProhibitedFromScientificUse:true,technicalTrajectoryObservedLength:pp.rows.length,technicalMeasuredRootPlies:pair.map(x=>x.ply),technicalRootSelectionRule:"latest consecutive nonterminal post-move root pair",trajectoryReplayExact:true,adjacentSuccessorBindingExact:true,overlappingWindowSemanticsExact:true,repeatedRawIdentityTimeIndexSemanticsExact:true,technicalProductionIndependentRootReconstructionExact:true,technicalProductionIndependentFamilyExact:true,technicalProductionIndependentBectLevelExact:true,implementationSeparation:true,m5Denominator:"sum uniqueTransitionCount[d], d=0..4",freshScientificSeedAccess:false,stage1SeedAccess:false,stage2SeedAccess:false,protectedDepth10Access:false};
const result={...technicalCore,deterministicCoreSha256:sha(technicalCore),stageDisposition:"STAGE0-PASS"};
const out=path.join(DOC,"results/stage-0-v2");fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,"STAGE_0_TECHNICAL_RESULT.json"),JSON.stringify(result,null,2)+"\n");console.log(`BECT_STAGE0_V2_RESULT=${JSON.stringify(result)}`);
