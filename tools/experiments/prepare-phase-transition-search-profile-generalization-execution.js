...
function parseArgs(argv) {
  const options = {
    ...
    output: "artifacts/phase-transition/search-profile-generalization-v2/execution-lock.json",
  };
...
function gitIgnored(relativePath, cwd) {
  ...
  const result = spawnSync("git", ["check-ignore", "--quiet", "--no-index", `${normalized}/.e019-ignore-probe`], { cwd });
...
function pythonEnvironment(pythonCommand) {
  try {
    ...
    const packages = JSON.parse(execFileSync(pythonCommand, ["-c", "import json,numpy,pandas; print(json.dumps({'numpy':numpy.__version__,'pandas':pandas.__version__}))"], { encoding: "utf8" }).trim());
...
function validateEnvironment(policy, environment, preregistration) {
  const errors = [];
  try { E019.validatePreregistration(preregistration); }
  catch (error) { errors.push(`Invalid E-019 preregistration: ${error.message}`); }
  if (environment.githubActions === true) errors.push("Formal E-019 execution lock cannot be prepared in GitHub Actions.");
  ...
  if (environment.corpusRootIgnored !== true) errors.push("Formal E-019 corpus root is not ignored by git.");
  ...
  if (policy.formalExecutionAllowed !== true || policy.status !== "approved-awaiting-local-lock") {
    errors.push("E-019-specific formal authorization is required before preparing an execution lock.");
  }
...
function prepare(options) {
  ...
  const lock = {
    ...
    approval: {
      ...
      note: "This lock may exist only after explicit E-019 authorization. It does not authorize GitHub Actions formal execution.",
    },
    errors,
  };
...
module.exports = { atomicWrite, collectEnvironment, gitIgnored, parseArgs, prepare, pythonEnvironment, sha256, validateEnvironment };
