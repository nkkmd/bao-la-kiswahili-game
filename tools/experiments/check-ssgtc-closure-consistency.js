...
function excludes(path, needles) {
  const text = read(path);
  for (const needle of needles) {
    if (text.includes(needle)) fail(`${path}: stale/prohibited text remains: ${JSON.stringify(needle)}`);
  }
  return text;
}
...
if (formal.formalEndpoints.reachableRawStatesThroughDepth8 !== 24848) fail("formal raw-state count mismatch");
if (formal.formalEndpoints.graphTransitionOccurrencesParentDepth0Through7 !== 25648) fail("formal transition count mismatch");
if (formal.formalEndpoints.gameTreeNodeOccurrencesThroughDepth8 !== 30941) fail("formal tree-node count mismatch");
if (formal.formalEndpoints.gameTreeEdgeOccurrencesThroughDepth8 !== 30940) fail("formal tree-edge count mismatch");
if (formal.formalEndpoints.stateSetSha256 !== STATE_HASH) fail("formal state hash mismatch");
if (formal.formalEndpoints.transitionSetSha256 !== TRANSITION_HASH) fail("formal transition hash mismatch");
if (formal.formalEndpoints.treeOccurrenceSetSha256 !== TREE_HASH) fail("formal tree hash mismatch");
...
if (formal.interpretationBoundary.symmetryReducedCountingAuthorized !== false) fail("symmetry-reduced counting must remain unauthorized");
