...
const {
  ...
  sourceFileHashes,
  summarizeGames,
} = require("./run-position-typology-stage2-confirmation.js");

function parseArgs(argv) {
  const options = {
    input: "artifacts/local/position-typology/stage2-mtaji-confirmation-v1",
    output: null,
  };
...
function main() {
  ...
  if (manifest.formalExperiment !== true
    ...
    throw new Error("Stage 2 confirmation manifest boundary mismatch");
  }
  if (manifest.preregistrationId !== spec.preregistrationId) {
    throw new Error("Preregistration id mismatch");
  }
  if (manifest.candidateDefinitionHash !== spec.candidateDefinition.requiredHash) {
    throw new Error("Candidate definition hash mismatch");
  }
  ...
  const specPathKey = "doc/position-typology/preregistration/STAGE_2_MTAJI_CONFIRMATION_SPEC.json";
  ...
  const report = {
    ...
    status: "stage2-mtaji-confirmation-verification-passed",
    passed: true,
    formalExperiment: true,
    exploratory: false,
    candidateDefinitionHash: manifest.candidateDefinitionHash,
    ...
    checks: {
      ...
      candidateDefinitionHash: "passed",
