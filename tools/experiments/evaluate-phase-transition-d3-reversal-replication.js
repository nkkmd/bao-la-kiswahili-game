...
function logChoose(n, k) {
  ...
  let value = 0;
  for (let i = 1; i <= m; i += 1) value += Math.log(n - m + i) - Math.log(i);
  return value;
}
...
function exactMcNemarTwoSided(n10, n01) {
  ...
  const logs = [];
  for (let k = 0; k <= lower; k += 1) logs.push(logChoose(n, k) - n * LOG2);
...
function evaluate(config, pairs) {
  E020.validatePreregistration(config);
  ...
  const checks = {
    ...
    directionLegacyGreaterThanPhase2: counts.n01 > counts.n10,
  };
  let decision;
  if (!checks.exactPairCount || !checks.minimumDiscordantPairs) decision = "inconclusive";
  else if (checks.exactMcNemarAlpha && checks.directionLegacyGreaterThanPhase2) decision = "confirmed";
  ...
  return {
    ...
    rates: {
      P2: p2Rate,
      LG: lgRate,
      pairedRiskDifferencePhase2MinusLegacy: p2Rate === null || lgRate === null ? null : p2Rate - lgRate,
    },
    discordantOddsRatioLegacyOverPhase2: counts.n10 === 0 ? null : counts.n01 / counts.n10,
    discordantOddsRatioLabel: counts.n10 === 0 && counts.n01 > 0 ? "Infinity" : null,
    exactMcNemarTwoSidedP: pValue,
    checks,
    interpretationBoundary: "A confirmed E-020 result applies only to hard / bao / depth 3 and does not alter E-019/H17 or establish a general depth interaction.",
  };
}
...
function fisherExactTwoSided(a, b, c, d) {
  for (const value of [a, b, c, d]) {
    if (!Number.isInteger(value) || value < 0) throw new Error("Fisher counts must be non-negative integers");
  }
  const row1 = a + b;
  const row2 = c + d;
  const col1 = a + c;
  ...
  for (let x = minimumA; x <= maximumA; x += 1) {
    ...
    if (logProbability <= observedLog + 1e-12) included.push(logProbability);
  }
...
function structuralComparison(p2Candidates, p2Expansion, lgCandidates, lgExpansion) {
  ...
  if (values.some((value) => !Number.isInteger(value) || value < 0)) throw new Error("Structural counts must be non-negative integers");
  if (p2Expansion > p2Candidates || lgExpansion > lgCandidates) throw new Error("Expansion count cannot exceed candidate count");
  const p2Rate = rate(p2Expansion, p2Candidates);
  const lgRate = rate(lgExpansion, lgCandidates);
  const legacyOverPhase2RiskRatio = p2Rate === null || lgRate === null || p2Rate === 0 ? null : lgRate / p2Rate;
  return {
    counts: { p2Candidates, p2Expansion, lgCandidates, lgExpansion },
    rates: { P2: p2Rate, LG: lgRate },
    riskDifferencePhase2MinusLegacy: p2Rate === null || lgRate === null ? null : p2Rate - lgRate,
    legacyOverPhase2RiskRatio,
    riskRatioLabel: p2Rate === 0 && lgRate !== null && lgRate > 0 ? "Infinity" : null,
...
function parseArgs(argv) {
  const options = {
    ...
    pairs: null,
    output: "artifacts/local/phase-transition-d3-reversal-replication-v1/evaluation",
  };
  ...
  if (!options.pairs) throw new Error("--pairs is required; evaluator does not generate data");
  return options;
}
