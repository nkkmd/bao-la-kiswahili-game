...
function parseArgs(argv) {
  const options = { output: EXTENSION.output, force: false, status: false };
...
function experimentConfig() {
  return {
    ...
    replicates: EXTENSION.replicates,
    games: EXTENSION.replicates * CONDITIONS.length,
    baseOpeningSeed: EXTENSION.baseOpeningSeed,
    maxPly: EXTENSION.maxPly,
    opening: {
      policy: "seeded-uniform-legal",
      plies: EXTENSION.openingPlies,
    ...
    extensionPolicy: {
      purpose: "exploratory CBE exposure support only",
...
function extensionGame(config, replicateIndex, conditionIndex) {
  const game = base.runGame(config, replicateIndex, conditionIndex);
  const newGameId = `nmt-s1e-r${String(replicateIndex).padStart(3, "0")}-${game.conditionId}`;
...
function status(output) {
  ...
  return {
    ...
    output,
    completedGameFiles,
    expectedGames: EXTENSION.replicates * CONDITIONS.length,
  };
}
...
function main() {
  ...
  if (source.sourceTreeDirty) {
    throw new Error("Instrumented source tree is dirty; commit/stash relevant source changes before Stage 1 extension generation");
  }
  ...
  for (let replicateIndex = 0; replicateIndex < config.replicates; replicateIndex += 1) {
    for (let conditionIndex = 0; conditionIndex < config.conditions.length; conditionIndex += 1) {
      ...
      if (!game) {
        game = extensionGame(config, replicateIndex, conditionIndex);
        atomicWriteJson(file, game);
      }
      games.push(game);
      console.log(`[Namua-Mtaji Stage 1 Extension] ${games.length}/${config.games} ${game.gameId}`);
    }
  }
  ...
  const manifest = {
    ...
    status: "stage1-exposure-support-extension-complete",
    formalExperiment: false,
    scientificInferenceAuthorized: false,
    exploratoryAnalysisAuthorized: true,
    confirmatoryReuseAllowed: false,
