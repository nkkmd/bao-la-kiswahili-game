...
const DEFAULT_SPEC = path.join(REPO_ROOT, "doc/symmetry-isomorphic-positions/preregistration/STAGE_1_FORMAL_SPEC_V2.json");
const DEFAULT_AUTH = path.join(REPO_ROOT, "doc/symmetry-isomorphic-positions/preregistration/STAGE_1_AUTHORIZATION_V2.json");
const DEFAULT_OUTPUT = path.join(REPO_ROOT, "doc/symmetry-isomorphic-positions/results/STAGE_1_PRODUCTION_RESULT_V2.json");
const VERIFIER_PATH = path.join(__dirname, "verify-symmetry-isomorphism-stage1-formal-v2.js");
...
function verifyAuthorization(specPath, authPath) {
  ...
  if (auth.studyId !== "SIP-STUDY1" || auth.stageId !== spec.stageId || auth.authorized !== true) throw new Error("Stage 1 v2 not authorized");
  if (auth.specSha256 !== sha256Buffer(specBytes)) throw new Error("Authorization/spec SHA256 mismatch");
  if (auth.scientificOutcomeGeneratedBeforeAuthorization !== false) throw new Error("Invalid v2 authorization chronology");
  const p = spec.paths;
  ...
  for (const [key, file] of Object.entries(files)) {
    if (sha256File(file) !== expected[key]) throw new Error(`Stage 1 v2 source hash mismatch: ${key}`);
  }
...
function validateFreshScope(domain, candidateId, strataNames, depth) {
  ...
  for (const key of testedKeys) {
    ...
    if (terminalSource && transformed.winner !== swapWinner(source.winner, candidateId)) record("G7", "WINNER-MISMATCH", { stateKey: key });
...
function rebuildOracleGraph(oracle) {
  const rootKey = oracle.rootResult.stateKey;
  const rootRow = oracle.stateRows.find((row) => row.stateKey === rootKey);
  if (!rootRow) return { complete: false, technicalFailure: { type: "ORACLE-ROOT-ROW-MISSING" } };
  const root = { ...clone(rootRow.ruleState), reason: "", turn: 1 };
  if (rawStateKey(root) !== rootKey) return { complete: false, technicalFailure: { type: "ORACLE-ROOT-RAW-KEY-MISMATCH", expected: rootKey, actual: rawStateKey(root) } };
  const states = new Map(); const edges = []; const queue = [];
  function add(state) {
    const key = rawStateKey(state);
    if (!states.has(key)) { states.set(key, clone(state)); queue.push(key); }
    return key;
  }
  add(root);
  let cursor = 0;
  while (cursor < queue.length) {
    const sourceKey = queue[cursor++]; const source = states.get(sourceKey);
    if (source.winner !== null) continue;
    const moves = exactMtajiMoves(source);
    if (!moves.length) return { complete: false, technicalFailure: { type: "ORACLE-REBUILD-NONTERMINAL-NO-MOVE", sourceKey } };
    for (const move of moves) {
      const applied = applyMtajiGuardFree(source, move);
      if (applied.status !== "TERMINATED") return { complete: false, technicalFailure: { type: applied.status, sourceKey, moveKey: exactMoveKey(move) } };
      const targetKey = add(applied.state);
      edges.push({ sourceKey, move: clone(move), moveKey: exactMoveKey(move), targetKey });
    }
  }
  const stateKeys = sorted(states.keys());
  const edgeRows = sorted(edges.map((edge) => `${edge.sourceKey}\t${edge.moveKey}\t${edge.targetKey}`));
  const identity = {
    stateCount: states.size,
    edgeCount: edges.length,
    stateSetSha256: sha256Text(stateKeys.join("\n")),
    transitionSetSha256: sha256Text(edgeRows.join("\n")),
  };
  const expected = oracle.domain;
  const passed = identity.stateCount === expected.stateCount
    && identity.edgeCount === expected.edgeCount
    && identity.stateSetSha256 === oracle.identities.stateSetSha256
    && identity.transitionSetSha256 === oracle.identities.transitionSetSha256;
  return { complete: passed, technicalFailure: passed ? null : { type: "ORACLE-RAW-GRAPH-IDENTITY-MISMATCH", identity }, states, edges, identity };
}
...
function validateOracle(oracle, candidateId) {
  const base = rebuildOracleGraph(oracle);
  if (!base.complete) return {
    decision: "NON-ESTIMABLE", technicalFailure: base.technicalFailure,
    gates: { G9: "NON-ESTIMABLE", G10: "NON-ESTIMABLE", G11: "NON-ESTIMABLE" },
    mismatchCount: 0, mismatchCounts: {}, mismatches: [], baseGraphIdentity: base.identity || null,
  };
  const mismatches = []; const mismatchCounts = {}; const gateFailures = new Set();
  const fail = (gate, type, data = {}) => { gateFailures.add(gate); mismatchCounts[type] = (mismatchCounts[type] || 0) + 1; if (mismatches.length < 100) mismatches.push({ gate, type, ...data }); };
  const outgoingBySource = new Map();
  for (const edge of base.edges) { if (!outgoingBySource.has(edge.sourceKey)) outgoingBySource.set(edge.sourceKey, []); outgoingBySource.get(edge.sourceKey).push(edge); }
  const transformedStates = new Map(); const transformedEdges = [];
  for (const [sourceKey, source] of base.states) {
    if (!isApplicable(source, candidateId)) { fail("G2", "ORACLE-NOT-APPLICABLE", { stateKey: sourceKey }); continue; }
    const transformed = transformState(source, candidateId); const transformedKey = rawStateKey(transformed);
    if (transformedStates.has(transformedKey) && rawStateKey(transformedStates.get(transformedKey)) !== transformedKey) fail("G3", "ORACLE-STATE-COLLISION", { sourceKey, transformedKey });
    transformedStates.set(transformedKey, transformed);
    if (!validRuleState(transformed)) fail("G2", "ORACLE-TRANSFORMED-INVALID", { stateKey: sourceKey });
    if (rawStateKey(transformState(transformed, candidateId)) !== sourceKey) fail("G3", "ORACLE-STATE-INVERSE", { stateKey: sourceKey });
    if ((source.winner !== null) !== (transformed.winner !== null)) fail("G6", "ORACLE-TERMINAL-MISMATCH", { stateKey: sourceKey });
    if (source.winner !== null && transformed.winner !== swapWinner(source.winner, candidateId)) fail("G7", "ORACLE-WINNER-MISMATCH", { stateKey: sourceKey });
    if (source.winner !== null) continue;
    const sourceEdges = outgoingBySource.get(sourceKey) || [];
    const mappedKeys = sourceEdges.map((edge) => exactMoveKey(transformMove(edge.move, candidateId)));
    const targetMoves = exactMtajiMoves(transformed);
    if (!sameSet(mappedKeys, targetMoves.map(exactMoveKey))) fail("G4", "ORACLE-LEGAL-SET-MISMATCH", { stateKey: sourceKey, mappedKeys: sorted(mappedKeys), targetKeys: sorted(targetMoves.map(exactMoveKey)) });
    for (const edge of sourceEdges) {
      const mappedKey = exactMoveKey(transformMove(edge.move, candidateId));
      const targetMove = targetMoves.find((candidate) => exactMoveKey(candidate) === mappedKey);
      if (!targetMove) { fail("G5", "ORACLE-MAPPED-MOVE-ILLEGAL", { stateKey: sourceKey, moveKey: edge.moveKey, mappedKey }); continue; }
      const transformedApplied = applyMtajiGuardFree(transformed, targetMove);
      if (transformedApplied.status !== "TERMINATED") return {
        decision: "NON-ESTIMABLE", technicalFailure: { type: transformedApplied.status, stateKey: transformedKey },
        gates: { G9: "NON-ESTIMABLE", G10: "NON-ESTIMABLE", G11: "NON-ESTIMABLE" },
        mismatchCount: Object.values(mismatchCounts).reduce((a,b) => a+b, 0), mismatchCounts, mismatches, baseGraphIdentity: base.identity,
      };
      const sourceTarget = base.states.get(edge.targetKey);
      const expected = rawStateKey(transformState(sourceTarget, candidateId)); const actual = rawStateKey(transformedApplied.state);
      if (actual !== expected) fail("G5", "ORACLE-COMMUTATION-MISMATCH", { stateKey: sourceKey, moveKey: edge.moveKey, expected, actual });
      transformedEdges.push({ sourceKey: transformedKey, moveKey: mappedKey, targetKey: actual });
    }
  }
  if (transformedStates.size !== base.states.size) fail("G3", "ORACLE-STATE-BIJECTION-COUNT", { transformed: transformedStates.size, expected: base.states.size });
  if (transformedEdges.length !== base.edges.length) fail("G5", "ORACLE-EDGE-BIJECTION-COUNT", { transformed: transformedEdges.length, expected: base.edges.length });
  const structuralMismatchCount = Object.values(mismatchCounts).reduce((a,b) => a+b, 0);
  if (structuralMismatchCount) return {
    decision: "NOT-VALIDATED", technicalFailure: null,
    gates: { G9: "N/A-STRUCTURAL-MISMATCH", G10: "N/A-STRUCTURAL-MISMATCH", G11: "N/A-STRUCTURAL-MISMATCH" },
    counts: { states: transformedStates.size, edges: transformedEdges.length }, mismatchCount: structuralMismatchCount, mismatchCounts, mismatches, baseGraphIdentity: base.identity,
  };
  const solution = solveCompleteGraph(transformedStates, transformedEdges);
  const solutionRows = new Map(oracle.stateRows.map((row) => [row.stateKey, row]));
  for (const [sourceKey, source] of base.states) {
    const row = solutionRows.get(sourceKey);
    if (!row) { fail("G9", "ORACLE-SOLUTION-ROW-MISSING", { stateKey: sourceKey }); continue; }
    const transformedKey = rawStateKey(transformState(source, candidateId)); const result = solution.solved.get(transformedKey);
    if (!result) { fail("G9", "ORACLE-SOLUTION-MISSING", { stateKey: sourceKey }); continue; }
    if (result.status !== row.status) fail("G9", "ORACLE-VALUE-MISMATCH", { stateKey: sourceKey, expected: row.status, actual: result.status });
    const expectedWinner = row.absoluteWinner === null ? null : swapWinner(row.absoluteWinner, candidateId);
    if (result.absoluteWinner !== expectedWinner) fail("G9", "ORACLE-ABSOLUTE-WINNER-MISMATCH", { stateKey: sourceKey, expectedWinner, actualWinner: result.absoluteWinner });
    if (result.dtf !== row.dtf) fail("G10", "ORACLE-DTF-MISMATCH", { stateKey: sourceKey, expected: row.dtf, actual: result.dtf });
    const sourceMoves = source.winner === null ? exactMtajiMoves(source) : [];
    const mappedOptimal = row.optimalMoveKeys.map((key) => {
      const move = sourceMoves.find((candidate) => exactMoveKey(candidate) === key);
      return move ? exactMoveKey(transformMove(move, candidateId)) : `UNRESOLVED:${key}`;
    });
    if (!sameSet(mappedOptimal, result.optimalMoveKeys)) fail("G11", "ORACLE-OPTIMAL-MOVE-MISMATCH", { stateKey: sourceKey, expected: sorted(mappedOptimal), actual: sorted(result.optimalMoveKeys) });
  }
  ...
  return {
    ...
    gates: { G9: [...gateFailures].includes("G9") ? "FAIL" : "PASS", G10: [...gateFailures].includes("G10") ? "FAIL" : "PASS", G11: [...gateFailures].includes("G11") ? "FAIL" : "PASS" },
    counts: { states: transformedStates.size, edges: transformedEdges.length, recurrentSccs: solution.recurrentSccs.length }, mismatchCount, mismatchCounts, mismatches, baseGraphIdentity: base.identity,
  };
}
...
function main() {
  ...
  const outcomes = scientific.map((item) => {
    ...
    const exactOracle = oracleByCandidate[item.candidateId];
    return { ...item, fresh, exactOracle, provisionalDecision: combineDecision(fresh, exactOracle), G12: "PENDING-INDEPENDENT-VERIFICATION" };
  });
  ...
  const result = {
    schemaVersion: 2, studyId: "SIP-STUDY1", stageId: binding.spec.stageId,
    supersedesInvalidatedStageId: "SIP-S1-FORMAL-2026-08-24-v1",
    ...
    correctionBoundary: "v2 changes only exact-oracle raw graph reconstruction after v1 IDENTITY positive-control failure. Candidate transforms, applicability, fresh domain, seeds, root selection, depth and decision labels are unchanged.",
  ...
  process.stdout.write(`${JSON.stringify({ stageId: result.stageId, summary: result.summary, controls: controls.map((row) => ({ candidateId: row.candidateId, decision: row.controlDecision, freshMismatches: row.fresh.mismatchCount, oracleMismatches: row.exactOracle.mismatchCount })), outcomes: outcomes.map((row) => ({ outcomeId: row.outcomeId, candidateId: row.candidateId, decision: row.provisionalDecision, freshMismatches: row.fresh.mismatchCount, oracleMismatches: row.exactOracle.mismatchCount })) }, null, 2)}\n`);
}

if (require.main === module) main();
module.exports = { combineDecision, rebuildOracleGraph, solveCompleteGraph, validateFreshScope, validateOracle };
