#!/usr/bin/env python3
"""Apply the unchanged inherited Category-A pipeline to the Stage 2 formal corpus.

This is locked preprocessing. It does not inspect Mtaji morphology labels or fit
any new candidate definition.
"""
...
base = load_module("nmt_stage2_candidate_base", BASE_PATH)
v2 = base.v2
...
def validate_identity(manifest: dict, input_dir: Path):
    if manifest.get("formalExperiment") is not True:
        raise RuntimeError("Stage 2 formal boundary mismatch")
    if manifest.get("scientificInferenceAuthorized") is not True:
        raise RuntimeError("Stage 2 inference authorization mismatch")
    if manifest.get("exploratoryAnalysisAuthorized") is not False:
        raise RuntimeError("Stage 2 exploratory boundary mismatch")
    if manifest.get("morphologyEffectInspectionAuthorized") is not True:
        raise RuntimeError("Stage 2 morphology authorization mismatch")
    config = manifest.get("config", {})
    if config.get("study") != "namua-mtaji-temporal-transition":
        raise RuntimeError("Unexpected study identity")
    if config.get("stage") != "stage2-formal-confirmation":
        raise RuntimeError("Unexpected Stage 2 identity")
    conditions = config.get("conditions", [])
    if len(conditions) != 1 or conditions[0].get("id") != "P2-D2":
        raise RuntimeError("Stage 2 formal condition must be P2-D2 only")
...
def load_formal(input_dir: Path):
    ...
    if len(observations) != int(manifest["summary"]["observations"]):
        raise RuntimeError("Formal observation count mismatch")
...
def main():
    ...
    observations, games, manifest = load_formal(input_dir)
    ...
    report = {
        "schemaVersion": 1,
        "status": "stage2-formal-inherited-category-a-pipeline-complete",
