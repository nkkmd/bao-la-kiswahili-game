...
const {
  sourceFileHashes,
  summarizeGames,
} = require("./run-namua-mtaji-transition-smoke.js");

function parseArgs(argv) {
  const options = {
    input: "artifacts/local/namua-mtaji-transition/stage0-smoke-v1",
    output: null,
  };
...
function validateGameShape(game) {
  const required = [
    "gameId", "gameIndex", "seed", "conditionId", "condition", "configHash",
    "observations", "moves", "historicalTrajectoryHash", "ruleTrajectoryHash",
    "seatCanonicalTrajectoryHash", "temporalOutcome",
  ];
...
function main() {
  ...
  if (manifest.formalExperiment !== false
    ...
    || manifest.candidateDiscoveryAuthorized !== false) {
    throw new Error("Stage 0 smoke scientific boundary mismatch");
  }
  ...
  if (!sourceHashesMatch) throw new Error("Current source-file hashes differ from smoke provenance");

  const report = {
    schemaVersion: 1,
    generatedAt: new Date().toISOString(),
    status: "stage0-technical-smoke-verification-complete",
    passed: true,
...
module.exports = { gameFiles, parseArgs, validateGameShape, verifyGame };
