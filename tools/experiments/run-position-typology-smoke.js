...
function parseArgs(argv) {
  const options = {
    ...
    maxPly: 100,
    openingPlies: 8,
    output: "artifacts/local/position-typology/stage0-smoke-v1",
    force: false,
    status: false,
  };
...
function experimentConfig(options) {
  return {
    ...
    stage: "stage0-instrumentation-smoke",
    schemaVersion: 1,
    games: options.games,
...
function runGame(config, gameIndex) {
  ...
  const gameId = `ptyp-s0-${String(gameIndex).padStart(4, "0")}`;
  const observations = [];
  const moves = [];
  ...
  const openingStateKey = openingObservation.identity.ruleStateKey;
...
function status(output) {
  ...
  const games = fs.existsSync(gamesDir) ? fs.readdirSync(gamesDir).filter((name) => /^game-\d+\.json$/.test(name)).length : 0;
...
function main() {
  ...
  for (let gameIndex = 0; gameIndex < config.games; gameIndex += 1) {
    ...
    games.push(game);
    console.log(`[position-typology smoke] ${gameIndex + 1}/${config.games} ${game.gameId}`);
  }
  const summary = summarizeGames(games);
  const manifest = {
    schemaVersion: 1,
    generatedAt: new Date().toISOString(),
    status: "stage0-instrumentation-smoke-complete",
    formalExperiment: false,
    clusteringAuthorized: false,
    output,
    config,
    configHash,
    provenance: source,
