...
function replayGame(engine,game,item,spec){
 ...
 for(let ply=0;ply<game.moves.length;ply++){
  const row=game.moves[ply];C.assert(row.ply===ply&&row.stateHash===C.sha256(state),'MOVE_SEQUENCE_MISMATCH');
  const legal=engine.E.moveVariantsForSearch(state),serialized=JSON.stringify(row.move);C.assert(legal.some(m=>JSON.stringify(m)===serialized),'RECORDED_MOVE_ILLEGAL');
  C.assert(row.competitor===(state.player===item.jevPlayer?'jev-direct':'ai-gen4'),'COMPETITOR_MISMATCH');
  state=C.clone(engine.E.applyMoveForSearch(state,row.move).state);C.assert(row.afterHash===C.sha256(state),'AFTER_STATE_HASH_MISMATCH');
 }
 C.assert(C.sha256(state)===C.sha256(game.state),'GAME_STATE_MISMATCH');return state;
}
function loadGames(O,spec,engine){
 const all=[...games(O,'pilot'),...games(O,'formal')],rows=[];
 for(const item of all){const file=gameFile(item.id);if(!fs.existsSync(file))continue;const g=read(file);replayGame(engine,g,item,spec);rows.push({id:g.id,stage:g.stage,openingId:g.openingId,jevPlayer:g.jevPlayer,status:g.status,winner:g.state.winner,plies:g.moves.length,pause:g.pause||null});}
 return rows;
}
function report(O,spec,ledger,engine,status){
 const rows=loadGames(O,spec,engine),value={studyId:C.protocol.id,status,createdAt:new Date().toISOString(),spec,budget:ledger.summary(),
...
async function choose(engine,state,{isJev,client,logicalId,stage,allowRetry}){
 ...
 const started=performance.now(),result=engine.baseline(state),elapsedMs=performance.now()-started;
 const applied=engine.E.applyMoveForSearch(state,result.move);
...
async function runStage(O,spec,ledger,engine,client,stage,{allowRetry=false,log=console.log}={}){
 const prior=report(O,spec,ledger,engine,'VALIDATED');
 if(stage==='formal')C.assert(prior.pilot.terminalGames===4&&prior.pilot.incompletePairs===0,'PILOT_NOT_TECHNICALLY_COMPLETE');
 for(const item of games(O,stage)){
  const file=gameFile(item.id);let game;
  if(fs.existsSync(file)){game=read(file);replayGame(engine,game,item,spec);}else{
   game={studyId:C.protocol.id,specHash:spec.specHash,id:item.id,stage,openingId:item.openingId,jevPlayer:item.jevPlayer,initialState:C.clone(item.initialState),
    createdAt:new Date().toISOString(),environment:environment(),state:C.clone(item.initialState),moves:[],status:'PAUSED',pause:null};C.atomicJSON(file,game);
  }
  if(game.status==='TERMINAL')continue;
  if(['UNRESOLVED','ENGINE-LIMIT'].includes(game.status))return report(O,spec,ledger,engine,game.status);
  try{
   while(game.state.winner===null&&game.moves.length<C.protocol.outcomes.maxPlies&&game.state.reason!=='relay-limit'){
    ...
    if(fs.existsSync(df)){row=read(df);const legal=engine.E.moveVariantsForSearch(game.state);C.assert(row.stateHash===C.sha256(game.state)&&legal.some(m=>JSON.stringify(m)===JSON.stringify(row.move)),'PENDING_DECISION_INVALID');}
    else{
     const d=await choose(engine,game.state,{isJev:game.state.player===item.jevPlayer,client,logicalId:id,stage,allowRetry});
     row={ply,stateHash:C.sha256(game.state),competitor:d.competitor,move:d.move,moveKey:d.moveKey,afterHash:d.afterHash,
      candidateId:d.candidateId||null,candidateSetHash:d.candidateSetHash||null,requestHash:d.requestHash||null,remote:d.remote||null,search:d.search||null};
     C.atomicJSON(df,row);
    }
   ...
   game.status=finalStatus(game);C.atomicJSON(file,game);if(game.status!=='TERMINAL')return report(O,spec,ledger,engine,game.status);
   log(item.id+': TERMINAL '+game.moves.length+' plies');
...
async function main(){
 ...
 if(cmd==='status'||cmd==='verify'){
  const ledger=new B.Ledger(ledgerDir,spec.specHash),r=report(O,spec,ledger,engine,cmd==='verify'?'VERIFIED':'STATUS');console.log(JSON.stringify(r,null,2));return;
 }
 C.assert(['run','resume'].includes(cmd)&&['pilot','formal'].includes(args[1])&&args[2]==='--live'&&args.length===3,'COMMAND_INVALID');
 const stage=args[1];requireAuthorization(stage,spec);C.assert(process.env.TYPESAFE_API_KEY,'TYPESAFE_API_KEY_REQUIRED');
 const release=B.lock(ledgerDir);
 try{
  const ledger=new B.Ledger(ledgerDir,spec.specHash),client=R.createClient(ledger,resultsRoot,process.env.TYPESAFE_API_KEY);
  const r=await runStage(O,spec,ledger,engine,client,stage,{allowRetry:cmd==='resume'});console.log(JSON.stringify(r,null,2));
 }finally{release();}
}
...
module.exports={environment,checkEnvironment,verifyOpenings,verifyManifest,specification,games,replayGame,report,choose,runStage};
