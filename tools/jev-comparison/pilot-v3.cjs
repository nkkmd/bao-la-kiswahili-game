...
const R = require("./reviewed-client.cjs");
const ID = "pilot-matches-v3";
const DIAGNOSTIC_HASH = R.DIAGNOSTIC_HASH;
...
function inspect() {
  const env = require("./pilot-v2.cjs").inspect();
  C.assert(env.addonHash === "8851b1c461095d4c7987aeba7a93a95dfe37663e675b63935432515cb8b51070", "確認済みの予備対局版と一致しません。");
  const responseManifest = read(path.join(__dirname, "response-diagnostic-manifest.json"));
  C.assert(hash(responseManifest) === "ba73a733a1ad2477b64d52e1868662b769a4695872db7c92b115cf4a643bc80a", "確認済みの応答診断版と一致しません。");
  for (const [file, expected] of Object.entries(responseManifest.files)) C.assert(path.basename(file) === file && C.sha256(fs.readFileSync(path.join(__dirname, file))) === expected, "応答診断の固定ファイルが一致しません。");
  const manifest = read(path.join(__dirname, "pilot-v3-manifest.json"));
  C.assert(manifest.id === ID, "予備対局の識別子が一致しません。");
  for (const name of ["pilot-v3.cjs", "reviewed-client.cjs", "PILOT_V3.md"]) C.assert(typeof manifest.files[name] === "string", "追加ファイルが不完全です。");
  ...
  const diagnostic = read(path.join(env.output, "diagnostics/response-diagnostic-v1.json"));
  C.assert(hash(diagnostic) === DIAGNOSTIC_HASH && diagnostic.observation.legacyValidation === "PASS", "確認済みの応答診断結果と一致しません。");
  C.assert(hash(read(path.join(env.output, "pilot-v2/games/pilot-v2-pair-0-game-0.json"))) === "f288e2f7c0920678337c063645e2cfec79cafc1afdd0543f4b1462115403474a", "旧条件の棋譜が停止時点と一致しません。");
  return { ...env, addonHash: hash(manifest), root: path.join(env.output, "pilot-v3") };
}

function planFor(engine, addonHash) {
  const openings = C.fixtures(engine, SETTINGS.pilotPairs, SETTINGS.fixtureSeed + 1000);
  const plan = { schema: "bao-jev-pilot-v3-plan", id: ID, experimentId: SETTINGS.experimentId,
    addonHash, originalKitHash: "de638ab06d987e483e992f6d17ba867f13d9273ea184f73415571e3b7cf6fbda",
    diagnosticHash: DIAGNOSTIC_HASH, settings: SETTINGS, errorPolicy: "strict-validation-reject-and-fallback-log-v1", openingsHash: hash(openings),
    games: openings.flatMap((opening, pair) => [0, 1].map(seat => ({
      id: "pilot-v3-pair-" + pair + "-game-" + seat, pair, seat,
...
function saveSummary(env, plan, ledger, status, log) {
  ...
  const report = { schema: "bao-jev-pilot-v3-summary", createdAt: new Date().toISOString(), id: ID,
    ...
    budget: ledger.summary(), resumeReview: ledger.resumeReview || null, games,
    interpretation: "厳格な応答検証と異常時の代替探索を用いる予備対局です。以前の棋譜・診断・正式測定とは合算しません。" };
...
async function runMatches(env, plan, ledger, client, { shouldPause = () => false, choose = decide,
  ...
  try {
    for (const item of plan.games) {
      const file = path.join(env.root, "games", item.id + ".json");
      const game = fs.existsSync(file) ? read(file) : { schema: "bao-jev-pilot-v3-game", ...C.clone(item),
      ...
      if (game.pauseReason === "API-FALLBACK-STREAK") return saveSummary(env, plan, ledger, "PILOT-V3-PAUSED", log);
      if (complete(game.status)) continue;
      if (shouldPause()) return saveSummary(env, plan, ledger, "PILOT-V3-PAUSED", log);
      active = game; activeFile = file;
      C.atomicJSON(file, game);
      let streak = failureStreak(game);
      if (streak >= 3) { game.pauseReason = "API-FALLBACK-STREAK"; game.status = "PAUSED"; C.atomicJSON(file, game); return saveSummary(env, plan, ledger, "PILOT-V3-PAUSED", log); }
      while (game.state.winner === null && game.moves.length < SETTINGS.maxGamePlies && !shouldPause()) {
        ...
        if (streak >= 3) return saveSummary(env, plan, ledger, "PILOT-V3-PAUSED", log);
      }
      game.status = finalStatus(game);
      C.atomicJSON(file, game);
      active = null; activeFile = null;
      if (!complete(game.status)) return saveSummary(env, plan, ledger, "PILOT-V3-PAUSED", log);
      saveSummary(env, plan, ledger, "PILOT-V3-IN-PROGRESS", log);
    }
    return saveSummary(env, plan, ledger, "PILOT-V3-COMPLETE", log);
  } catch (e) {
...
async function main() {
  ...
  if (command === "help") { console.log("無課金確認: node tools/jev-comparison/pilot-v3.cjs preflight\n確認後の予備対局再開: node tools/jev-comparison/pilot-v3.cjs run --live --acknowledge-reviewed-resume"); return; }
  C.assert((["preflight", "status"].includes(command) && args.length === 1) || (command === "run" && args.length === 3 && args[1] === "--live" && args[2] === "--acknowledge-reviewed-resume"),
    "preflight、status、またはrun --live --acknowledge-reviewed-resumeを指定してください。");
  const env = inspect(), release = B.lock(env.ledgerDir);
  try {
    const ledger = new R.ReviewedLedger(env.ledgerDir, L.binding());
    if (command === "status") { console.log(JSON.stringify({ budget: ledger.summary(), resumeReview: ledger.resumeReview || null }, null, 2)); return; }
    if (!ledger.resumeReview) {
      C.assert(ledger.halted && ledger.lastHaltHash === R.TARGET_HALT && ledger.lastHaltReason === "invalid-response",
        "今回確認した停止履歴と一致しません。");
      R.checkReviewBase(ledger);
    }
    else C.assert(!ledger.halted, "新たな停止はこの再開許可で解除できません。");
    ...
    ledger.resumeReviewed(plan.planHash, env.addonHash, true);
    ...
    const client = R.createClient(ledger, env.output, process.env.TYPESAFE_API_KEY);
