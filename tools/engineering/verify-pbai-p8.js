...
const C=require('./lib/pbai-p8-common.js');
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
C.requireSupervision();
const {equalSavedNumber}=require('./lib/pbai-p8-json-number.js');
...
for(const name of ['development','development-matches','validation','validation-matches','holdout-matches']) {
  ...
  for(const [index,r] of source.rows.entries()) {
    ...
    if(labels) {
      ...
      assert.equal(stable(a),stable(label.teacher));equalSavedNumber(C.clip(a.stats.rootScore/1024),label.target);
      equalSavedNumber(B.fast.evaluate(r.state,r.state.player),label.staticScore);teacherRoots++;
    }
...
for(const name of ['development','validation']) {
 ...
 for(let i=0;i<data.length;i++) {
  ...
  equalSavedNumber(evalRows[i].scores.previous,C.finish(x=>C.referenceLogic(C.model(),x),s,p));
  equalSavedNumber(evalRows[i].scores.logic,optimized.evaluate(s,p));
 }
 const eq=C.read(path.join(C.OUT,name+'-equivalence.json'));
 assert.equal(eq.rows.length,data.length*2);
 for(const row of eq.rows){const a=structuredClone(row.previous),b=structuredClone(row.optimized);delete a.stats.elapsedMs;delete b.stats.elapsedMs;assert.equal(JSON.stringify(a),JSON.stringify(b));equivalenceRows++;}
}
C.write(path.join(C.OUT,'replay-verification.json'),{passed:true,sourceRoots,teacherRoots,matchGames,matchPlies,equivalenceRows,teacherRecomputation:'fixed depth only',timedMatchesRerun:false});
