...
const C=require('./lib/pbai-p10-common.js');
...
for(const f of files.filter(f=>/^.+-pair-\d+\.json$/.test(f))) {
  ...
  for(const g of pair.games) {
    ...
    for(const [n,p] of g.plies.entries()) {
      assert.equal(p.seed,(880000001+Object.keys(C.SPEC.matches).indexOf(pair.name)*10000000+pair.index*1000+g.side*200+n)>>>0);
