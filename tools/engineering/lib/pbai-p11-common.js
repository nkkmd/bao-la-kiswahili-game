...
const DOC=path.join(ROOT,'doc/ai-engineering/public-ai-improvement-program-11');
const OUT=path.join(ROOT,'artifacts/pbai-p11/run');
const SPEC=read(path.join(DOC,'SPEC.json'));
function model(){const bytes=fs.readFileSync(path.join(ROOT,'doc/ai-engineering/public-ai-improvement-program-9/MODEL.json'));if(sha(bytes)!==SPEC.modelSha256)throw Error('Frozen model mismatch');return JSON.parse(bytes);}
function requireSupervision(){if(process.env.PBAI_P11_SUPERVISED!=='1')throw Error('Use PBAI-P11 supervisor');}
let knownCache;
function knownIdentities(){
 if(!knownCache){
  knownCache=JSON.parse(require('node:zlib').gunzipSync(fs.readFileSync(path.join(ROOT,'doc/ai-engineering/public-ai-improvement-program-9/KNOWN_IDENTITIES.json.gz')))).rows;
  const script="import tarfile,json; t=tarfile.open('artifacts/pbai-p9/final-evidence.tar.gz'); rows=[]\nfor m in t.getmembers():\n if m.name.endswith('-source.json'):\n  v=json.load(t.extractfile(m)); rows+=v['rows']+v['audit']\nprint(json.dumps(rows))";
  knownCache.push(...JSON.parse(require('node:child_process').execFileSync('python',['-c',script],{cwd:ROOT,maxBuffer:32*1024*1024,encoding:'utf8'})));
  const priorScript=script.replace('artifacts/pbai-p9/final-evidence.tar.gz','artifacts/pbai-p10/incident/interrupted-raw-evidence.tar.gz');
  knownCache.push(...JSON.parse(require('node:child_process').execFileSync('python',['-c',priorScript],{cwd:ROOT,maxBuffer:32*1024*1024,encoding:'utf8'})));
 }
 return knownCache;
}
