...
function search(actor,state,seed,options,deadlineMs=10000){
 ...
 return new Promise((resolve,reject)=>{
  ...
  const worker=new Worker(path.join(__dirname,'pbai-p9-worker.cjs'),{workerData:{actor},resourceLimits:{maxOldGenerationSizeMb:256}});
  let done=false;
  ...
  worker.postMessage({type:'search',id:seed,seed,state,level:'hard',options});
 });
}
