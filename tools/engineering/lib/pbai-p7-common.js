...
const DOC=path.join(ROOT,'doc/ai-engineering/public-ai-improvement-program-7');
const OUT=path.join(ROOT,'artifacts/pbai-p7/run');
...
function requireSupervision(){if(process.env.PBAI_P7_SUPERVISED!=='1')throw Error('Use PBAI-P7 supervisor');}
function priorIdentities() {
  const rows=P6.read(path.join(P6.DOC,'KNOWN_IDENTITIES.json')).rows.concat(read(path.join(DOC,'P6_IDENTITIES.json')).rows);
...
module.exports={...P6,DOC,OUT,SPEC,model,select,requireSupervision};
