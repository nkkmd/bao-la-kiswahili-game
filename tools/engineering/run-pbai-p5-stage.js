"use strict";
const C=require("./lib/pbai-p5-common.js");
const fs=require("node:fs"),assert=require("node:assert/strict"),cp=require("node:child_process");
const {performance}=require("node:perf_hooks");
if(fs.existsSync(C.OUT+"/RESOURCE_STOP.json"))throw Error("PBAI-P5 closed by resource HOLD; measurement restart is not authorized");
if(process.env.PBAI_P5_SUPERVISED!=="1")throw Error("Use the PBAI-P5 supervisor");
const flag="pbaiC011LightweightTransitions";
const configs={development:{rootBase:822000001,roots:16,gameBase:826000001,pairs:4},validation:{rootBase:823000001,roots:16,gameBase:827000001,pairs:8},holdout:{rootBase:824000001,roots:32,gameBase:828000001,pairs:128}};
...
function allSelections(){return [...read(C.DOC+"/KNOWN_DATA_EXCLUSIONS.json").rows,...fs.readdirSync(C.OUT,{recursive:true}).filter(p=>String(p).endsWith("-source.json")).flatMap(p=>read(C.OUT+"/"+p).rows)];}
