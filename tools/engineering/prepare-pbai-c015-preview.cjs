...
// 同じオリジンで再試験しても、前のコピーのキャッシュを再利用しない。
const serviceWorkerPath = path.join(target, 'service-worker.js');
const serviceWorker = fs.readFileSync(serviceWorkerPath, 'utf8');
const cacheDeclaration = /const CACHE = "[^"]+";/;
if (!cacheDeclaration.test(serviceWorker)) throw Error('キャッシュ設定が見つかりません');
const previewCache = 'bao-la-kiswahili-preview-' + crypto.randomUUID();
fs.writeFileSync(serviceWorkerPath, serviceWorker.replace(cacheDeclaration, `const CACHE = "${previewCache}";`));
...
const manifest = { purpose: 'PBAI-C015-v1 hard-only preview', sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim(),
  ...
  createdAt: new Date().toISOString(), physicalDeviceVerified: false, previewOnly: true, previewCache, transformations: ['ai-release.js: PBAI_C015_ENABLED set to true', 'privacy.html copied to extensionless privacy for local HTTP serving', 'service-worker.js: unique cache name per preview'], files };
