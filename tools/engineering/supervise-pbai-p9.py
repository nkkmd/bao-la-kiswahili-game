"""PBAI-P9 consume-once orchestration with an external process-group deadline."""
import os, sys, time, json, signal, hashlib, pathlib, subprocess, platform
ROOT=pathlib.Path(__file__).resolve().parents[2]
DOC=ROOT/'doc/ai-engineering/public-ai-improvement-program-9'
OUT=ROOT/'artifacts/pbai-p9/run'
...
def run_child(args,deadline,wall_deadline,env):
    ...
    try:
        while child.poll() is None:
            if time.monotonic()>=deadline or time.time()>=wall_deadline:raise TimeoutError('PBAI-P9 resource deadline')
            time.sleep(.1)
...
def main():
    ...
    if (ROOT/'artifacts/pbai-p9/RUN_STARTED.json').exists():raise RuntimeError('Archived run already consumed')
    if OUT.exists() and any(OUT.iterdir()):raise RuntimeError('Raw output is not empty; do not restart')
    lock=verify_lock()
    browser=read(DOC/'BROWSER_PREPARATION.json')
    ...
    env=os.environ.copy();env.update({'PBAI_P9_SUPERVISED':'1','OPENBLAS_NUM_THREADS':'1','OMP_NUM_THREADS':'1','MKL_NUM_THREADS':'1'})
    ...
    def node(*args):command(['node','--max-old-space-size=2048','tools/engineering/run-pbai-p9-stage.js',*args])
    outcome='TECHNICAL-INVALID / HOLD';details=None
    try:
        node('pilot')
        if not read(OUT/'pilot-matches.json')['operation']:outcome='OPERATION-GATE-FAIL / HOLD'
        else:
            node('holdout');node('low');outcome='PENDING-FINAL-AUDIT'
        command(['node','--max-old-space-size=2048','tools/engineering/verify-pbai-p9.js'])
        command([sys.executable,'tools/engineering/audit-pbai-p9.py'])
    ...
    write(OUT/'FINAL_RESULT.json',{'program':'PBAI-P9','decision':outcome,'error':details,'elapsedSeconds':time.monotonic()-start,'finishedUnix':time.time(),'release':'NO-RELEASE / KEEP-AI-GEN3','lowGenerated':(OUT/'low-matches-source.json').exists(),'holdoutGenerated':(OUT/'holdout-matches-source.json').exists()})
