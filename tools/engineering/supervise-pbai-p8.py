"""PBAI-P8 consume-once orchestration with an external process-group deadline."""
import os, sys, time, json, signal, hashlib, pathlib, subprocess, platform
ROOT=pathlib.Path(__file__).resolve().parents[2]
DOC=ROOT/'doc/ai-engineering/public-ai-improvement-program-8'
...
def run_child(args,deadline,wall_deadline,env):
    ...
    try:
        while child.poll() is None:
            if time.monotonic()>=deadline or time.time()>=wall_deadline:raise TimeoutError('PBAI-P8 resource deadline')
            time.sleep(.1)
...
def main():
    ...
    env=os.environ.copy();env.update({'PBAI_P8_SUPERVISED':'1','OPENBLAS_NUM_THREADS':'1','OMP_NUM_THREADS':'1','MKL_NUM_THREADS':'1'})
    ...
    def node(*args):command(['node','--max-old-space-size=2048','tools/engineering/run-pbai-p8-stage.js',*args])
    ...
    try:
        ...
        command(['node','--max-old-space-size=2048','tools/engineering/verify-pbai-p8.js'])
        command([sys.executable,'tools/engineering/audit-pbai-p8.py'])
    ...
    write(OUT/'FINAL_RESULT.json',{'program':'PBAI-P8','decision':outcome,'error':details,'elapsedSeconds':time.monotonic()-start,'finishedUnix':time.time(),'release':'NO-RELEASE / KEEP-AI-GEN3','validationGenerated':(OUT/'validation-source.json').exists(),'holdoutGenerated':(OUT/'holdout-matches-source.json').exists()})
