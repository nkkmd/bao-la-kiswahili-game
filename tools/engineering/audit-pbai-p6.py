...
SPEC=json.loads((ROOT/'doc/ai-engineering/public-ai-improvement-program-6/SPEC.json').read_text())
...
def run():
    ...
    for split in ('development','validation'):
        ...
        for kind in ('baseline','logic','linear','mlp'):
            ...
            assert hashlib.sha256((OUT/f'{kind}-model.json').read_bytes()).hexdigest()==report['modelHashes'][kind] if kind!='baseline' else True
        logic=summaries['logic']; baseline=summaries['baseline']
        gates={'mse':logic[0]<=baseline[0]*1.10,'phaseSpeed':all(v<=.90 for v in logic[1].values()),'p95Speed':logic[2]<=1.10,'tactical':logic[3]==0,'baselineTactical':baseline[3]==0,'export':load(OUT/'export-verification.json')['passed']}
