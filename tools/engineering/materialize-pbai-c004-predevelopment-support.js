...
const SPEC_PATH = "doc/ai-engineering/public-ai-improvement-program-1/candidates/PBAI-C004-v1-predevelopment-support-spec.json";
...
function summarizeRoot(root) {
  ...
  if (!transition) throw new Error("Missing D2-to-D3 transition");
...
function main() {
  ...
  const eligible = measured.filter((root) => root.topSetDisjoint);
  const selected = eligible.slice(0, spec.supportMeasurement.targetMaximum);
  const stableCanonical = measured.filter(
    (root) => !root.topSetDisjoint && !root.canonicalBestChanged,
  );
  ...
  const report = {
    ...
    measurement: {
      searchSemantics: spec.supportMeasurement.searchSemantics,
      depths: spec.supportMeasurement.depths,
      evaluationProfile: spec.supportMeasurement.evaluationProfile,
      quiescenceDepth: spec.supportMeasurement.quiescenceDepth,
      orderQuiescenceCaptures: spec.supportMeasurement.orderQuiescenceCaptures,
      rootsMeasured: measured.length,
      d23TopSetDisjoint: eligible.length,
      d23TopSetOverlap: measured.length - eligible.length,
      deterministicCanonicalBestStableAmongOverlap: stableCanonical.length,
      phaseSupport: {
        namuaEligible: eligible.filter((root) => root.phase === "namua").length,
        mtajiEligible: eligible.filter((root) => root.phase === "mtaji").length,
      },
    },
    targetSupport: {
    ...
    selectedTargetRefs: selected.map(({ seed, ply, phase, rawKey }) => ({ seed, ply, phase, rawKey })),
