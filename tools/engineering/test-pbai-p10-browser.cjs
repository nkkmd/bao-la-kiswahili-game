...
const out = path.resolve(__dirname, '../../artifacts/local/pbai-p10-browser', engine);
...
const report = { engine, sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  ...
  transformations: ['candidate: isolated expert adapter; test cache name', 'rollback: original hard-only adapter; different test cache; hard retained'],
};
const server = http.createServer((req, res) => {
  ...
  if (name === 'ai-release.js' && mode === 'candidate') body = Buffer.from(require('./lib/pbai-p10-adapter.cjs')(body.toString()));
  if (name === 'service-worker.js') body = Buffer.from(body.toString().replace('bao-la-kiswahili-v35', mode === 'candidate' ? 'bao-la-kiswahili-p10-candidate' : 'bao-la-kiswahili-p10-rollback'));
...
(async () => {
  ...
  try {
    ...
    await check('配信元への通信不能時に候補資産をキャッシュから利用', async () => {
      ...
      assert(await page.evaluate(async () => !!(await (await caches.open('bao-la-kiswahili-p10-candidate')).match('./logic-evaluator.js'))));
    ...
    await check('設定無効化とキャッシュ更新で基準AIへ切戻す', async () => {
      ...
      await eventually(() => page.evaluate(async () => { const names = await caches.keys(); return names.includes('bao-la-kiswahili-p10-rollback') && !names.includes('bao-la-kiswahili-p10-candidate') && !names.includes('bao-la-kiswahili-v33'); }));
