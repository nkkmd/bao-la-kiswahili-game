"""PBAI-P6 consume-once orchestration with an external process-group deadline."""
...
def run_child(args,deadline,wall_deadline,env):
    ...
    try:
        while child.poll() is None:
            if time.monotonic()>=deadline or time.time()>=wall_deadline:raise TimeoutError('PBAI-P6 resource deadline')
            time.sleep(.1)
...
def main():
    ...
    env=os.environ.copy();env.update({'PBAI_P6_SUPERVISED':'1','OPENBLAS_NUM_THREADS':'1','OMP_NUM_THREADS':'1','MKL_NUM_THREADS':'1'})
    ...
    def node(*args):command(['node','--max-old-space-size=2048','tools/engineering/run-pbai-p6-stage.js',*args])
    outcome='TECHNICAL-INVALID / HOLD';details=None
    try:
        node('generate','train');node('generate','development')
        command([sys.executable,'tools/engineering/train-pbai-p6.py']);node('export')
        ...
        command(['node','--max-old-space-size=2048','tools/engineering/verify-pbai-p6.js'])
        command([sys.executable,'tools/engineering/audit-pbai-p6.py'])
    ...
    write(OUT/'FINAL_RESULT.json',{'program':'PBAI-P6','decision':outcome,'error':details,'elapsedSeconds':time.monotonic()-start,'finishedUnix':time.time(),'release':'NO-RELEASE / KEEP-AI-GEN3','validationGenerated':(OUT/'validation-source.json').exists(),'holdoutGenerated':(OUT/'holdout-matches-source.json').exists()})
