...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;try{
 ...
 const report=JSON.parse(await page.locator('#result').textContent());report.browserVersion=browser.version();report.commit=process.env.GITHUB_SHA||null;report.pageErrors=errors;
