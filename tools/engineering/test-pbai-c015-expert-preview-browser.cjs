...
const temp = fs.mkdtempSync(path.join(require('node:os').tmpdir(), 'bao-expert-browser-'));
const dirs = Object.fromEntries(['candidate', 'rollback'].map(m => [m, path.join(temp, m)]));
const manifests = Object.fromEntries(Object.entries(dirs).map(([m, d]) => [m, require('./prepare-pbai-c015-expert-preview.cjs').prepare(d, m)]));
const out = path.resolve(__dirname, '../../artifacts/local/pbai-c015-expert-preview-browser', engine);
...
const report = { engine, sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  ...
  previewManifests: manifests, transformations: ['generated device preview served verbatim', 'generated rollback served verbatim'],
};
const server = http.createServer((req, res) => {
  ...
  if (candidateUnavailable && name === 'ai-candidate.js') return res.writeHead(404).end();
  const file = path.join(dirs[mode], name === 'privacy' ? 'privacy.html' : name);
...
async function start(page, level = 'expert') {
...
(async () => {
  ...
  try {
    ...
    await check('公開画面でexpert候補が着手し診断に識別子を保存', async () => {
      ...
      assert.equal(await page.evaluate(() => BaoReleaseConfig.displayIdentity('expert').releaseId), 'PBAI-P11-EXPERT-PREVIEW');
    ...
    await check('思考開始後の新規対局で古い応答を適用しない', async () => {
      ...
      await page.selectOption('#difficulty', 'expert'); await page.click('#start-game');
    ...
    await check('配信元への通信不能時に候補資産をキャッシュから利用', async () => {
      ...
      assert(await page.evaluate(async cache => !!(await (await caches.open(cache)).match('./logic-evaluator.js')), manifests.candidate.cache));
    ...
    await check('設定無効化とキャッシュ更新で基準AIへ切戻す', async () => {
      ...
      await eventually(() => page.evaluate(async cachesToCheck => { const names = await caches.keys(); return names.includes(cachesToCheck.rollback) && !names.includes(cachesToCheck.candidate) && !names.includes('bao-la-kiswahili-v33'); }, { rollback: manifests.rollback.cache, candidate: manifests.candidate.cache }));
    ...
    assert.equal(await page.evaluate(() => BaoReleaseConfig.searchOptions('hard').pbaiC015LogicGate), true);
    ...
    for (const missing of ['model', 'candidate']) await check(missing + '未取得時の公開画面は基準AIで着手', async () => {
      const c = await browser.newContext({ reducedMotion: 'reduce', serviceWorkers: 'block' });
      const p = await c.newPage(); modelUnavailable = missing === 'model'; candidateUnavailable = missing === 'candidate';
      ...
      modelUnavailable = false; candidateUnavailable = false; await c.close();
    });
    report.passed = true;
