"use strict";
const C=require('./lib/pbai-p9-common'),{search}=require('./lib/pbai-p9-search');
...
async function run(){
 ...
 for(const [index,r] of rows.entries()){
  ...
  for(const side of (index%2?[1,0]:[0,1])){
   ...
   for(let n=0;n<C.SPEC.search.maxMatchPlies&&s.winner===null;n++){
    const actor=s.player===side?'logic':'baseline',seed=r.seed+n,options=C.options(config.maxDepth,config.timeLimitMs);
    const a=await search(actor,s,seed,options,C.SPEC.resources.requestDeadlineMs);
    assert.equal(a.type,'result');assert.equal(a.id,seed);assert.equal(a.positionKey,B.A.stateKey(s));
    ...
    const p={actor,seed,move:a.move,stats:a.stats,elapsedMs:a.wallTimeMs};plies.push(p);
 ...
 const operation=overrun.logic<=overrun.baseline+C.SPEC.gates.overrunRateIncreaseMax;
