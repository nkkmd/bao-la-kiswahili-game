...
SPEC=json.loads((ROOT/'doc/ai-engineering/public-ai-improvement-program-9/SPEC.json').read_text())
...
def run():
 summaries={};final=None
 for split in ('pilot','holdout','low'):
  ...
  operation=rates['logic']<=rates['baseline']+SPEC['gates']['overrunRateIncreaseMax'];assert operation==report['operation']
  ...
  if split=='holdout':
   ...
   final={**summaries[split],'clusters':len(clusters),'interval95':interval,'passed':passed,'decision':'STRENGTH-IMPROVED-IN-COLD-WORKER-DOMAIN' if passed else 'STRENGTH-IMPROVEMENT-UNCONFIRMED / HOLD'}
 assert load(OUT/'replay-verification.json')['passed']
 low=summaries.get('low');screen=None if low is None else low['operation'] and low['points']>=SPEC['gates']['lowDescriptivePointsMin'] and all(v>=SPEC['gates']['lowDescriptivePhasePointsMin'] for v in low['phasePoints'].values())
 with (OUT/'independent-audit.json').open('x') as f:json.dump({'passed':True,'summaries':summaries,'final':final,'lowDescriptiveScreen':screen,'physicalDeviceVerified':False},f,indent=2,allow_nan=False);f.write('\n')
