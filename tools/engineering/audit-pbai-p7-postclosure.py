"""P7終了後の記述的集計。凍結済み判定を変更しない。元の集計式を使用する。"""
"""保存済みの測定行から、JavaScript集計とは別に判定を検算する。"""
import json, pathlib, math, statistics, hashlib
import numpy as np
ROOT=pathlib.Path(__file__).resolve().parents[2]
OUT=ROOT/'artifacts/pbai-p7/run'
SPEC=json.loads((ROOT/'doc/ai-engineering/public-ai-improvement-program-7/SPEC.json').read_text())
...
def run():
    assert load(OUT/'FINAL_RESULT.json')['decision']=='TECHNICAL-INVALID / HOLD'
    checks=[]
    ...
    assert load(OUT/'postclosure-replay-verification.json')['passed']
    with (OUT/'postclosure-independent-audit.json').open('x') as f:json.dump({'passed':True,'diagnosticOnly':True,'formalDecisionUnchanged':True,'checks':checks,'final':final},f,indent=2);f.write('\n')
