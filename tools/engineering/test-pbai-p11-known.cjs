...
const {search}=require('./lib/pbai-p11-search');
const transform=require('./lib/pbai-p11-adapter.cjs');
const fixtures=require('./browser/pbai-p9/fixtures.json').rows;
const C=require('./lib/pbai-p11-common');
...
(async()=>{const results=[];
 ...
 assert.equal(C.knownIdentities().length,120331);
 const result={passed:true,knownOnly:true,timedMatchSeedsConsumed:false,physicalDeviceVerified:false,results};
 fs.mkdirSync('artifacts/local/pbai-p11',{recursive:true});fs.writeFileSync('artifacts/local/pbai-p11/preparation.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({passed:true,checks:results.length}));
