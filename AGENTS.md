...
このリポジトリでは、**サブエージェントおよびマルチエージェントへの作業委譲を行わない**。

- `spawn_agent`等のサブエージェント生成・委譲機能を使用しない。
- 並列化可能な作業であっても他のエージェントへ委譲せず、単一のメインエージェントで実行する。
- 調査、実装、検証、レビューを含むすべての作業を、原則として現在のメインエージェント自身が行う。
- サブエージェントを利用可能な環境でも、このリポジトリの作業では利用しない。
- ユーザーがその作業について明示的にサブエージェントの使用を指示した場合のみ例外とする。

Codex向けの機能設定は[`.codex/config.toml`](.codex/config.toml)でも無効化する。`AGENTS.md`は運用上の指示、`.codex/config.toml`は対応するCodex実行環境での機能設定として併用する。
