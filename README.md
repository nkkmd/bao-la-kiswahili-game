...
Research Generation 3の正式成果を設計入力とする新しい`PBAI-P3`は、`AI-GEN2` baselineと、`PBAI-C010-v1`だけからなるinitial candidate inventory、fresh split、prospective gate、baseline-only support protocolを結果確認前に固定しました。現在は`PBAI-P3-D` support / reachability auditだけが認可され、結果生成前のrun manifestと隔離toolingまで固定しています。候補実装、benefit benchmark、validation、release holdout、公開変更は承認されていません。Research Generation 4とは別のAI Engineering Programです。
