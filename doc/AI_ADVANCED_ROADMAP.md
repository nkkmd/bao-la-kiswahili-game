...
- [x] 開発用に現在局面をJSONとしてコピーする機能
- [x] AIの選択手と探索統計を含む診断形式
- [x] 人間が「この手はおかしい」と印を付けるローカル記録
- [x] 集めた局面を`test/`または`artifacts/`へ移すためのfixture変換入口

### 途中結果

2026-07-11に、外部送信を伴わない最小診断ループを実装した。

- `diagnostics.js`へversion付き局面形式、AI着手・探索統計の許可リスト、局面復元を追加した
- 現在局面のJSONコピー、直前AI着手の手動記録、記録JSONのコピー、全削除をUIへ追加した
- 記録はlocalStorageの`bao_ai_feedback_v1`へ最大50件保存し、自由記述、氏名、端末識別子、保存時刻を含めない
- 自動保存、サーバー送信、分析イベント送信は実装しない
- Service Workerの事前キャッシュとプライバシーポリシーを更新した

続いて`diagnostic-to-fixture.js`を追加し、単一snapshot、記録配列、配列内index指定から`human-review`ケースを生成できるようにした。生成物は必ず失敗するTODO assertionを含み、期待手、避ける手、人間目線の理由、必要探索深度をレビューするまで回帰へ追加できない。

Phase 10の実装基盤は完了した。実施状況は、実対局由来の局面収集とレビューがまだないため「進行中」のままとする。最低3件の違和感局面を収集し、探索不足／評価不足／期待手の誤認へ分類した後、Phase 12へ進むか判断する。

具体的な記録、export、fixture変換、深度比較、分類の手順は`AI_HUMAN_REVIEW_GUIDE.md`を参照する。
...
TT終局距離正規化は、勝敗値を保存時・読出時にply補正し、TTキーからplyを外す候補として比較した。固定深度4の32局面と`mtaji-depth-trap`深度7で、着手、nodes、cache hitsはすべて既定と同じだった。異なるplyから同一局面へ至る転置の増加を観測できず、正確性リスクに対する利益がないため不採用とする。

### Phase 11終了判断

- 採用: hard/expertの探索内評価キャッシュ、上限2,048 entries
- 保留: TT最善手優先、Quiescence捕獲順序
- 不採用: history heuristic、固定幅aspiration window、TT終局距離正規化
- 維持: 既定の捕獲優先順序、plyを含むTTキー、固定探索予算
- 次: Phase 10のローカル局面コピーと違和感記録を実装し、探索では解けない反例を収集する
...
1. Phase 10のローカル局面コピーと違和感記録を追加し、対人反例を蓄積する
2. 対人局面を探索深度別に診断し、探索不足と評価不足へ分類する
3. 深い探索でも解けない反例が複数集まった場合だけPhase 12へ進む
...
Phase 10の実装基盤は完了した。次の最小作業単位は実対局レビューである。

1. hardまたはexpertとの実対局で、違和感のあるAI着手を最低3件記録する
2. 記録JSONを`diagnostic-to-fixture.js`でレビュー用fixtureへ変換する
3. 各局面について期待手または避ける手と、Bao上の理由を人間が記述する
4. 深度2、4、6以上を比較し、探索を深くすると解けるか確認する
5. 探索で解ける局面はPhase 11の回帰へ、深くしても解けない局面はPhase 12の特徴量候補へ分類する
6. 期待手の根拠が不十分な局面は回帰へ採用せず、レビュー待ちとして保持する

実対局データが集まるまでは、既存評価重みの再調整やPhase 12の特徴量実装を先行しない。
