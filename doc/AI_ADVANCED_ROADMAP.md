...
次は、コピーした単一snapshotまたは記録配列を読み、戦術fixtureの雛形へ変換する開発ツールを追加する。期待手や正解理由は自動決定せず、人間が確認して追記する。
...
最初の着手としては、Phase 10のうちサーバー保存を伴わないローカル診断を行う。

1. 現在局面をルール関連フィールドだけのJSONとして取得する関数を追加する
2. 開発用UIから局面JSONをクリップボードへコピーできるようにする
3. AIの選択手、評価profile、探索統計を局面JSONと一緒に記録できる形式を定める
4. 「この手はおかしい」という印はlocalStorageへ明示操作で保存し、自動操作ログは収集しない
5. 保存データを`test/tactical.test.js`の局面へ変換できることを回帰テストで確認する
6. 個人情報、時刻、端末識別子、対局者名を保存しないことを文書化する

この最小単位では外部送信を実装しない。集めた局面は人間が内容を確認してから、開発用fixtureまたは戦術回帰へ追加する。
