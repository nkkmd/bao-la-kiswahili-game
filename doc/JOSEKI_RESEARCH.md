...
第一次研究の必須課題は完了した。P002外部検証、評価関数診断、MCTS再設計、第二次局面標本、人間棋譜照合は、完了を妨げない将来研究として`doc/joseki/JOSEKI_FUTURE_RESEARCH.md`へ移管した。
...
P002・P003について、自己対局勝数首位手とcross-method合意手の計24 traceを同じ定義で再生し、固定手後の冒頭連続強制捕獲局面数、全強制捕獲局面数、恒久的な探索値負転を集約した。ここでの首位は近似自己対局6条件の勝数で、真の最善手を意味しない。
...
両局面とも探索は合意手を430点以上高く評価したが、非合意手が自己対局勝数首位となった。P003ではfrontSafetyとNorth応手数が同値であるため、この2特徴は測定差の必要条件ではない。
...
以上から、frontSafety差、相手応手数差、強制捕獲系列長のいずれも単独の必要条件・十分条件として採用しない。ただし後続のdepth sweepでP002合意手の強制勝ちが見つかったため、この節の不一致を評価関数の誤りとは解釈しない。
...
## 19. P002・P003探索depth sweep

両局面をphase2・bao・quiescence depth 1でdepth 1〜8まで反復深化した。depth 1〜4の推奨手とroot値は既存Phase 4成果物8行と完全一致し、depth 5〜8を新規追加した。全16行が60秒上限内に完了した。

P002はdepth 1〜7でconsensus手を維持し、depth 8でroot値999991となった。候補別South値はconsensus手999992、自己対局勝数首位手276で、探索器はconsensus手から9 ply以内のSouth終局勝ちを検出した。

P003もdepth 1〜8でconsensus手を維持した。自己対局勝数首位手−consensus手の候補値差はdepth 2〜8で常に負で、depth 8では-220だった。P003だけdepth 9〜10へ延長し、推奨は引き続きconsensus手、値差は-260、-198だった。depth 10まで終局勝ちスコアや推奨切替はない。

この結果は「深くすれば自己対局勝数首位手へ切り替わる」という仮説をP002で反証し、P003でもdepth 10まで支持しない。自己対局勝数は後続方策の性能を含み、固定初手のminimax価値とは分離して扱う。

完全性監査:

- 基本sweep: 2局面×depth 1〜8、16/16完了、timeout 0
- 既存depth 1〜4照合: 8/8一致
- P003延長: depth 9〜10、2/2完了、timeout 0
- base verification hash: `1ec406f452a9b3972f27d999ad85fc2341ad04cf4c146ece72cd1a9d7ccfb1a3`
- extension verification hash: `f38735dfe924ff0f2467dc552df8e23d6c16cf9094d3e9bd10fdfd9176dce94d`
- 詳細は`doc/joseki/FORCED_DEPTH_SWEEP.md`と`doc/joseki/P003_DEPTH_EXTENSION.md`を参照

## 20. P002 depth 8勝ち系列の再構成

P002 depth 8の主変化を各plyで再探索し、8探索ply後にquiescence depth 1が読む唯一合法な捕獲を加えた。9手目でSouthが`front-empty`勝ちとなり、全着手・局面hash・最終state hashを再生検証した。Northの主変化4着は全て合法手1の強制応手だった。

保存済みconsensus手自己対局6条件と比較すると、bao-d3・d4は9手を完全一致してSouth勝となった。bao-d2・bao-v2-d2の敗戦はSouthの3手目、bao-d1の敗戦はSouthの5手目で勝ち系列から外れた。legacy-d2も3手目で同じ別手へ外れたが別経路でSouth勝となるため、再構成系列だけが唯一の勝ち筋とは断定しない。

したがってP002で3/6勝だった原因は固定したconsensus初手そのものではなく、浅い自己対局方策が後続の勝ち筋を常に維持しなかったことにある。これは現探索実装内の強い証拠だが、独立実装によるゲーム理論的証明ではない。事前候補基準は変更せず、P002を直ちに定石へ昇格しない。

完全性監査:

- root score: 999991、completed depth 8
- 8探索ply＋1 quiescence ply、South `front-empty`勝ち
...
P002・P003の全6候補について、固定自己対局勝数、最深共通探索の値と順位、有界強制終局、cross-method consensusを一表に統合した。固定自己対局は後続方策のストレス試験、深い探索は共通depth内の相対順位、有界終局は明示範囲内の評価器非依存証明、cross-methodは選択安定性として扱う。

P002 consensus手は固定自己対局3/6で2位だが、depth 8探索首位かつ9 ply South強制勝ちであり、`bounded-forced-win-supported`とした。P003 consensus手は固定自己対局4/6で2位、depth 11探索首位、13 ply有界終局は`unresolved`であり、`deep-search-supported-bounded-unresolved`とした。

判定優先度は「有界終局証明 → 深い共通depth探索 → cross-method安定性 → 固定方策自己対局」とする。ただしこれは異なる指標を単一得点へ合算する規則ではない。別ルール実装または人間対局検証がない限り、現エンジン内の有界勝ちがあっても`promotionEligible`はfalseとする。

完全性監査:

- 2局面、6候補、全証拠軸を入力成果物hashで固定
- 固定自己対局首位と深い探索首位の不一致行4件
- promotion eligible 0件
- P002 decision: `bounded-consensus-supported-no-promotion`
- P003 decision: `deep-consensus-supported-bounded-unresolved-no-promotion`
- verification hash: `02590595d67b54b6e8d468d1a0e865af93432d89bfe51e85e95d5d8269d6fd39`
- 詳細は`doc/joseki/EVIDENCE_AXES.md`を参照

## 26. P002 9手系列の人間向け盤面照合票

P002有界証明の9手について、各plyの着手前後盤面、reserve、合法手数、捕獲・relay要約、state hash、石数台帳を生成した。盤上・reserve・pendingの合計は全遷移で64石、Northの4応手は全て合法手1択、最終局面はNorth frontが全て0のSouth `front-empty`勝ちとなった。

照合票は同じ`public/engine.js`から生成しているため独立検証ではない。人間または別実装が記入するチェック欄を未記入で残し、P002の外部状態を`externally-unvalidated`とした。これは第一次研究の終了を妨げないが、`provisional-joseki`または`validated`への昇格を妨げる。
...
- replay hash: `795d16195e598c03a3e418587a8a05390dd59a4a3f902992383679ded2fd5bcd`
- 詳細は`doc/joseki/P002_HUMAN_REPLAY.md`を参照

## 27. 第一次研究の完了

計画の成功条件8件を全て満たしたため、第一次研究を`completed-without-provisional-joseki`として終了する。標準初期局面では一般定石候補なし、暫定定石0件、validated 0件である。候補が認定されなかったこと自体を、全応手脆弱性とAI条件不安定性を再現可能に示した研究成果とする。

RQ1〜RQ6の最終回答、条件付き候補の状態、成功条件、外部検証の扱いは`doc/joseki/JOSEKI_FIRST_STUDY_CONCLUSION.md`に固定した。16件の前提verificationを照合し、成功条件8/8、promotion eligible 0件を確認した。

将来研究は終了条件から分離し、`doc/joseki/JOSEKI_FUTURE_RESEARCH.md`へ移管した。今後新しい証拠が得られた場合は、第一次研究成果物を上書きせず、追試として追加する。

最終完全性:

- prerequisite verification: 16/16 passed
