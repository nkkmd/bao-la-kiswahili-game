...
Direct Policy Studyの判定は固定した`jev-1.13.0`、prompt、candidate表現、Direct Policy方式に限定する。Jev一般、別model version、別prompt、別統合方式へ一般化しない。[完了後の追加局面解析](jev-direct-policy-comparison/POSTHOC_POSITION_ANALYSIS_20260923.md)では、splitした7開幕の開始局面とAI-GEN4敗局中の41判断局面の強制勝敗、静的評価と実際の着手の違いを記録した。この探索的な知見は正式判定や公開AIの採用判断を変更しない。`NO-PRODUCTION-ADOPTION-FROM-THIS-STUDY`を維持する。
