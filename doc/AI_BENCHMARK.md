...
| 条件 | peak entries | evictions | 実評価計算 | cache hits |
| --- | ---: | ---: | ---: | ---: |
| holdout、上限50,000 | 357 | 0 | 2,120 | 479 |
| holdout、上限256 | 256 | 101 | 2,120 | 479 |

上限256でevictionされた局面は再利用されず、hit数と実評価計算は変わらなかった。より深いhard/expert探索へ余裕を持たせ、採用上限は2,048 entriesとする。

500ms確認:

| 開始phase | 勝敗 | 候補平均深度 | 既定平均深度 | 候補／既定timeout | cache peak／evictions |
| --- | ---: | ---: | ---: | ---: | ---: |
| namua | 両者2分 | 3.88 | 3.81 | 16／16 | 287／0 |
| mtaji | 両者1勝1敗 | 10.00 | 9.00 | 0／1 | 152／0 |

mtajiは1手で終局する短い2局のため勝率根拠には使わないが、同じ500msで候補だけが最大深度10を完了した。固定深度32局面の完全一致、評価計算削減、中規模16局、500msスモーク、cache容量確認を合わせ、評価キャッシュをhard/expertの既定へ採用する。easy/normalでは既定無効、hard/expertでは既定有効、上限2,048 entriesとする。

## Phase 11：TT終局距離正規化

終局距離を含む勝敗値をTT保存時・読出時にply補正し、TTキーから探索plyを外す候補を比較した。

| 条件 | 着手一致 | 既定nodes | 候補nodes | 既定／候補cache hits |
| --- | ---: | ---: | ---: | ---: |
| 初回16局面、深度4 | 16/16 | 6,465 | 6,465 | 同数 |
| holdout 16局面、深度4 | 16/16 | 5,097 | 5,097 | 同数 |
...
PBAI-P5はCOMPLETE / STRENGTH-IMPROVED-IN-FROZEN-DOMAIN。新規最終holdout512局で328勝184敗、勝点率64.0625％、cluster bootstrap 95％区間61.1328125〜66.9921875％となり、固定した100ms/D8の範囲で改善を確認した。全工程は約40分17秒で完了した。公開用configではhard/expertの候補を既定有効にし、main統合と配信確認を待っている。AI-GEN3への正式昇格は保留している。
