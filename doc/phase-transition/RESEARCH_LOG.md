...
- 候補群と対照群を終局まで `0–4 / 5–8 / 9–16 / 17+ ply` に層別化。
- forcing解除前兆率を各層で集計。
- 分析後窓8plyを確保できる9ply以上を直接比較対象とした。

### 結果

- A候補のforcing解除前兆6件は全て終局まで0–4ply。
- 終局まで9ply以上残るA候補8件では0件。
- 同条件の対照群では576/4062（14.2%）。
- 対照群は後窓8plyを要求するため0–4ply帯を含まず、この帯の直接比較は不能。

### 解釈の変更

- 「forcing解除前兆がA候補に独立して濃縮する」という解釈を撤回。
- 6件を終局近傍サブタイプとして再分類。
- 主たる戦略的相転移候補の比較では終局まで9ply以上を別集計する。
