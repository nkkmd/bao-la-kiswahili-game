...
主検証は候補plyの完全一致ではなく、条件ごとの捕獲分岐急拡大濃縮が再現するかで行う。候補ply近接性とアーキタイプ分布は副次評価とする。
...
次の主要検証はE-011のAI・depth頑健性である。E-011の結果はE-010の再判定には使用しない。
...
E-011では最大捕獲可能量の非対称化を副次評価項目として事前登録したが、主判定条件には含めない。
