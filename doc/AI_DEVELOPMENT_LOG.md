...
2026年9月23日に[AI-GEN4以降の改善候補](ai-engineering/AI_GEN4_FUTURE_IMPROVEMENT_OPTIONS_20260923.md)を記録し、最終精査で現行コード上の重複遷移、毎手Worker起動、現行モデルの教師条件を踏まえて提案を具体化しました。Workerの寿命と探索データの持越しを分け、教師・教材の改善案、各候補の見送り条件、棋力・速度・応答時間を分けた検証設計を追記しています。文書精査のみで、新規試験・学習・実装は行っていません。本書の過去の測定・判断や現行公開AIの構成は変更しません。
...
### 2026-07-10: Phase 11 TT最善手優先の初回比較

仮説:

Phase 2探索の着手順序は即時勝利、実捕獲量、TT最善手の順である。反復深化またはTTで得た最善手を即時勝利の次へ上げると、PVSの探索窓が早く狭まり、同一深度の探索nodeを削減できる可能性がある。

変更内容:

- `analyzeMove`へ実験用`ttMoveFirst`オプションを追加した。既定値は`false`とし、採用済み探索の意味を維持した
- `benchmark.js`へ競技者別のTT最善手優先フラグを追加した
- 固定深度の生成局面を比較する`phase11-compare.js`と回帰テストを追加した

測定条件:

- seed `20261000`
- namua/mtaji各8局面、opening plies 8
- 固定深度4、時間制限なし、既定`bao`評価、Quiescence深度1

結果:

- root着手は16/16局面で一致した
- 既定順序6465 nodesに対し、TT最善手優先は5401 nodesで、合計16.5%削減した
- 7局面でnode数が減り、悪化0局面、同数9局面だった
- Quiescence nodeは4222から3499へ減少した
- elapsed timeも減少したが、同一プロセス内の実行順とJITの影響を分離していないため採用根拠にはしない

判断:

- TT最善手優先を次段検証候補へ進める
- 既定値はまだ変更せず、未使用seedの固定深度比較とブラウザ相当時間の自己対戦を行う
- Quiescence捕獲順序は同時に入れず、TT最善手優先の判断後に独立比較する

### 2026-07-10: Phase 11 TT最善手優先のholdout
...
- root着手は16/16局面で一致した
- 既定順序5097 nodesに対し、TT最善手優先は5060 nodesで、削減率は0.7%だった
- 改善6局面、悪化1局面、同数9局面だった
- 初回とholdoutを合わせた32局面では11562から10461 nodes、合計9.5%削減だった
- 局面ごとの削減率中央値は0%で、効果は一部の大きな探索木へ偏った

判断:

- 未使用seedで効果が再現せず、事前に定めた中央値10%削減条件を満たさないため採用しない
- 時間制限自己対戦へは進めず、`ttMoveFirst`の既定値を`false`に保つ
- 比較オプションと測定ツールは、今後の着手順序診断用に残す
- 次はQuiescence Searchの捕獲手順序を独立して比較する

### 2026-07-10: Phase 11 Quiescence捕獲順序

仮説:

Quiescence Searchは合法な捕獲手をエンジン順に探索している。即時勝利と実捕獲量で順序付ければ、alpha-beta cutoffを早められる可能性がある。

変更内容:

- 実験用`orderQuiescenceCaptures`オプションを追加した。既定値は`false`とした
- 候補有効時は捕獲手を一度だけ適用し、即時勝利と全capture eventの合計で並べ、着手後局面を再帰探索へ再利用する
- `benchmark.js`と`phase11-compare.js`から競技者別に候補を指定できるようにした

固定深度結果:

- 初回seed: 6465から5803 nodes、10.2%削減、改善13、悪化0、同数3
- holdout: 5097から4777 nodes、6.3%削減、改善12、悪化0、同数4
- 32局面すべてでroot着手が一致した
- 合計削減率は8.5%だが、局面別中央値は初回5.6%、holdout 3.0%だった

時間制限スモーク:

- namua 100msは両者1勝1敗2分、候補平均深度2.20、既定2.08、timeoutは両者78
- mtaji 100msは両者2勝2敗、候補平均深度3.26、既定3.29、timeoutは両者26
...
### 2026-07-10: Phase 11 aspiration window

変更内容:

- 前回反復深化のroot scoreを中心に固定幅のalpha-beta窓を設定した
- fail-high／fail-low時は同じ深度を全幅で再探索し、boundを最終結果として扱わない
- 探索統計へ`aspirationResearches`を追加した
- `aspirationWindow`は0を既定とし、比較時だけ有効にした

結果:

- 幅50の初回16局面は全着手一致、6465から7196 nodesへ11.3%増加、全幅再探索34回だった
- 幅50のholdoutは全着手一致、5097から5544 nodesへ8.8%増加、全幅再探索36回だった
- 初回系列で幅100は11.5%、幅200は7.6%のnode増加だった

判断:

- 固定幅を広げても再探索コストを回収できず、不採用とする
- 時間制限自己対戦へ進めず、`aspirationWindow`の既定値を0に保つ
- 次は高価な評価と着手後局面の再利用を検討する

### 2026-07-10: Phase 11 評価キャッシュ初回比較

仮説:

反復深化と着手順序計算は同じ局面を複数回評価する。Bao評価は双方の捕獲手を実適用するため、同一探索内で評価値を再利用すれば、探索木を変えずに計算時間を削減できる可能性がある。

変更内容:

- 局面キーと評価視点をキーにする探索内評価キャッシュを追加した
- `evaluationRequests`、`evaluations`、`evaluationCacheHits`、`evaluationCacheStores`を探索統計へ追加した
- cacheは探索ごとに破棄し、既定上限を50000 entriesとした
- `evaluationCache`は実験オプションとし、既定値を`false`にした
- `phase11-compare.js`は局面ごとにbaselineとcandidateの先行順を交互にした

固定深度結果:

- 初回16局面は全着手・nodes一致、実評価計算3435から2615、23.9%削減、cache hit 820だった
- holdout 16局面も全着手・nodes一致、実評価計算2599から2120、18.4%削減、cache hit 479だった
- elapsed timeは初回約13.0%、holdout約1.6%減少したが、環境変動があるため補助指標とする

100msスモーク:

- namuaは候補1勝0敗3分、平均深度2.28。既定は0勝1敗3分、平均深度2.21
- mtajiは両者2勝2敗。候補平均深度3.78、既定3.54
- timeoutはnamuaで候補78／既定77、mtajiで候補34／既定38だった

判断:

- 探索意味を変えず、未使用seedでも実評価計算を18%以上削減したため中規模検証へ進める
- スモークは少局数なので採用根拠にはせず、複数seed・時間制限で勝敗、深度、timeout、最大思考時間を確認する
- 中規模検証完了まで既定値は変更しない

### 2026-07-10: Phase 11 評価キャッシュ中規模入口

測定条件:

- 未使用seed `20265000`、`20265010`、`20265100`、`20265110`
- namua/mtaji各8局、opening plies 8
- 150ms、最大深度8、最大40手
- 長時間プロセスの中断を避けるため4局ずつのchunkで保存した

結果:

- namuaは候補1勝0敗7分、既定0勝1敗7分。平均深度は候補2.52、既定2.50、timeoutは両者157だった
- mtajiは両者4勝4敗。平均深度は候補4.13、既定3.83、timeoutは両者66だった
- 最大思考時間はnamuaで候補175.5ms、既定174.4ms、mtajiで候補166.7ms、既定182.8msだった
- 候補の実評価計算はnamua 11623回で既定14561回、mtaji 6761回で既定7992回だった

判断:

- 中規模入口でも勝敗とtimeoutを悪化させず、mtajiの平均深度を改善した
...
### 2026-07-11: Phase 11 TT終局距離正規化と終了判断

変更内容:

- 勝ち値はTT保存時にplyを加え、読出時に現在plyを引く候補を追加した
- 負け値はTT保存時にplyを引き、読出時に現在plyを加える候補を追加した
- 候補有効時だけTTキーから探索plyを外した
- 即時勝利の優先を守る回帰を追加した

結果:

- 固定深度4の初回・holdout計32局面で、着手、nodes、cache hitsは既定と同じだった
- `mtaji-depth-trap`深度7でも両者1073 nodes、21 cache hitsで同じだった
- 異なるplyから同一局面へ到達するTT共有の増加を観測できなかった

判断:

- 正規化候補は理論上の安全策と回帰を備えたが、現在のBao探索で性能利益がないため不採用とする
