...
`CLOSED / FORMAL-COMPLETE`はrepository lifecycle status。scientific inferenceはpreregistered candidate-level labelsに限定する。
...
- **C1 unit-width occupancy fraction** は18/18 pairでMtaji > Namua、formal `CONFIRMED`。
- **C6 cumulative tree/RAW ratio** は18/18 pairでNamua > Mtaji、formal `CONFIRMED`。

両candidateともexact two-sided sign-test `p = 1/131072`、prospectively fixed Holm-Bonferroni gateをPASSした。
...
- sustained reply narrowing / forcing corridor
- branch reconvergence / decision funnel / tree-to-RAW compression

Principal upstreamはLGTGMIV F5/F2/F3/F4、F1はauxiliaryである。
