...
本研究は2026-07-15に完了した。不一致の原因はルールエンジンやbao評価のSouth/North座席依存ではなく、旧監査の鏡像変換（候補A）が、各プレイヤー自身から見たローカル座標である穴番号と方向を重ねて反転していたことだった。

BaoEngineではSouthとNorthの穴番号はいずれも各プレイヤー自身から見た左→右であり、物理的に向かい合う穴はindexの和が7になる。座席を交換する180度回転は、このローカル表現ではplayer-indexedな状態だけを交換し、`row`、`index`、`side`、`direction`を維持する候補Dとなる。特にnyumbaは両プレイヤーとも固定の`HOUSE = 4`である。候補Aの列反転はnyumbaの石をindex 3へ移しながら`houseOwned`だけを維持し、合法手生成と評価特徴量を変えていた。

固定seed `20260714` の旧200局面で得た候補比較は次のとおりである。

| 候補 | 合法手 | legacy評価 | bao評価 | 着手適用後状態 |
| --- | ---: | ---: | ---: | ---: |
| A（旧監査） | 170/200 | 200/200 | 135/200 | 738/830 |
| B | 43/200 | 200/200 | 135/200 | 0/830 |
| C | 64/200 | 200/200 | 200/200 | 0/830 |
| D（確定） | 200/200 | 200/200 | 200/200 | 830/830 |

候補Aの合法手不一致30件とbao評価不一致65件は、すべて状態変換段階のローカル座標反転で説明できた。両者の重複を除く81局面を`classification.json`へ分類し、合法手だけが崩れる初期局面と、評価だけが崩れる捕獲局面を最小回帰fixtureとして保存した。

候補Dによる最終監査では、10,000到達可能局面について二重変換、合法手、legacy評価、bao評価、bao-v2評価が完全一致し、それらに含まれる39,408合法着手の適用後状態も完全一致した。10,000/10,000のWilson 95%信頼区間下限は約99.962%だが、合否判定は計画どおり観測標本での100%一致とした。

結論として`public/engine.js`と`public/ai.js`の修正は行わない。先攻・後攻差研究の旧対局結果もエンジン修正の影響を受けないため、Phase 10の再実験は不要である。再現コマンドと座標規約は`tools/symmetry/README.md`、基準・分類・最終結果は`artifacts/namua-symmetry/`に保存した。
