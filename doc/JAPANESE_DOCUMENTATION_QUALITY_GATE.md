...
文体・粒度の最低基準は、第1〜第3段階の文書整備を`main`へ統合したコミット`68159b1dcdf1c9e0042b5933e4f88aca5dcec995`で固定する。整備範囲と検証結果は[`JAPANESE_DOCUMENTATION_PHASE3_PROGRESS.md`](JAPANESE_DOCUMENTATION_PHASE3_PROGRESS.md)を監査根拠とする。今後の文書は、この状態と同等以上の品質でなければならない。
...
### 12.1 PR・closureで残す監査記録

新規作成・大幅更新では、PR本文またはrepository内の監査記録に次を明記する。

- 監査対象の人間向け文書数とpath範囲
- 英語だけの人間向け見出しが0件であること
- 説明済みの許容例外を除く英語の完全な通常説明文が0件であること
- 壊れた相対リンクが0件であること
- 意図しないfenced code block変更が0件であること
- canonical情報の意図しない変更が0件であること
- 実行した関連test / validatorと結果
- 英語原文を残す場合の箇所、例外種別、理由、日本語での代替説明

単なるtypo修正など、意味・構造を変えない小変更では監査範囲を変更箇所へ限定できる。ただし、変更によって本ゲートを新たに破ってはならない。
...
- canonical情報に変更なし
- 英語残存は明示的な許容例外だけ
- 2026年9月5日に固定した基準品質と同等以上で、既存文書を後退させていない
- 対象範囲・例外・検証結果を追跡可能な形で記録している
