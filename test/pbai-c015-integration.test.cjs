...
function load({ model = true, enabled = false } = {}) {
  ...
  const script = name => {
    ...
    if (enabled && name === 'ai-config.js') text = text.replace('const PBAI_C015_ENABLED = false;', 'const PBAI_C015_ENABLED = true;');
    vm.runInContext(text, ctx, { filename: name });
  };
  ctx.importScripts = (...names) => names.forEach(script);
  script('ai-worker.js'); script('ai-config.js'); script('diagnostics.js');
...
test('既定は無効、有効時もhardだけ。切戻し後は元の設定', () => {
  ...
  for (const level of ['easy', 'normal', 'hard', 'expert']) {
    const a = off.BaoAIConfig.searchOptions(level), b = on.BaoAIConfig.searchOptions(level);
...
test('既知9局面で公開Worker・直接実行・切戻しが凍結参照に一致', () => {
  for (const enabled of [false, true]) for (const f of fixtures) {
    ...
    const options = { ...ctx.BaoAIConfig.searchOptions('hard'), maxDepth: 2, timeLimitMs: Infinity };
    ...
    const direct = ctx.BaoAI.analyzeMove(f.state, 'hard', rng(f.seed), options);
...
test('モデル読み込み失敗時はWorker・直接実行とも基準に戻る', () => {
  ...
  assert.equal(stable(ctx.BaoAI.analyzeMove(f.state, 'hard', rng(f.seed), options)), stable(r));
});
test('別難易度・profile・独自重みでは誤って有効にならない', () => {
  ...
  for (const [level, extra] of [['easy', {}], ['normal', {}], ['expert', {}], ['hard', { evaluationProfile: 'legacy' }], ['hard', { evaluationProfile: 'bao-v2' }], ['hard', { searchProfile: 'legacy' }], ['hard', { evaluationAdjustments: {} }]]) {
    assert.doesNotThrow(() => ctx.BaoAI.analyzeMove(fixtures[0].state, level, rng(1), { maxDepth: 1, timeLimitMs: 10, pbaiC015LogicGate: true, ...extra }));
  }
});
